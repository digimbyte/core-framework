...
namespace Nova
{
    ...
    public struct Alignment : System.IEquatable<Alignment>
    {
        ...
        ///  1 => <see cref="DepthAlignment.Back"/>
        ///  </param>
        public Alignment(float x, float y, float z) : this((int)Math.Sign(x), (int)Math.Sign(y), (int)Math.Sign(z)) { }
        ...
        ///  </param>
        /// <remarks><see cref="Z"/> will be assigned to <see cref="DepthAlignment.Center"/></remarks>
        public Alignment(float x, float y) : this((int)Math.Sign(x), (int)Math.Sign(y)) { }
        ...
        {
            int hash = 13;
            hash = (hash * 7) + ((int)X).GetHashCode();
            hash = (hash * 7) + ((int)Y).GetHashCode();
            hash = (hash * 7) + ((int)Z).GetHashCode();
            return hash;
        }
