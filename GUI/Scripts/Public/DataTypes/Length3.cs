...
namespace Nova
{
    ...
    public struct Length3 : IEquatable<Length3>
    {
        ...
        {
            Length3 length = new Length3();
            length.X = Length.FixedValue(value.x);
            length.Y = Length.FixedValue(value.y);
            length.Z = Length.FixedValue(value.z);
        ...
        {
            Length3 length = new Length3();
            length.X = Length.FixedValue(x);
            length.Y = Length.FixedValue(y);
            length.Z = Length.FixedValue(z);
            return length;
        }
        ...
        {
            Length3 length = new Length3();
            length.X = Length.Percentage(percent.x);
            length.Y = Length.Percentage(percent.y);
            length.Z = Length.Percentage(percent.z);
        ...
        {
            Length3 length = new Length3();
            length.X = Length.Percentage(x);
            length.Y = Length.Percentage(y);
            length.Z = Length.Percentage(z);
        ...
        public Length3(Length x, Length y, Length z)
        {
            ...
            // pretty gross, but the compiler complains
            XY = new Length2(x, y);
        }
        ...
        internal static Vector3 GetRawValue(Vector3 value, Length3 length, MinMax3 minMax, Vector3 relativeTo)
        {
            return new Vector3(Length.GetRawValue(value.x, length.X, minMax.X, relativeTo.x),
                               Length.GetRawValue(value.y, length.Y, minMax.Y, relativeTo.y),
                               Length.GetRawValue(value.z, length.Z, minMax.Z, relativeTo.z));
        }
