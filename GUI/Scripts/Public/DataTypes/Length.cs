...
namespace Nova
{
    ...
    public enum LengthType
    {
        ...
        /// </summary>
        Value = Internal.LengthType.Value,

        /// <summary>
        /// A relative measurement as a percentage.
        /// </summary>
        Percent = Internal.LengthType.Percent,
    };
    ...
    {
        internal const int SizeOf = Internal.Length.SizeOf;
        ...
        internal static float GetRawValue(float value, Length length, MinMax minMax, float relativeTo)
        {
            return length.Type == LengthType.Percent ? relativeTo == 0 ? length.Raw : minMax.Clamp(value) / relativeTo : minMax.Clamp(value);
        }
        ...
        {
            int hash = 13;
            hash = (hash * 7) + ((int)Type).GetHashCode();
            hash = (hash * 7) + Raw.GetHashCode();
            return hash;
        }
        ...
        {
            return Type == LengthType.Value ? $"{Raw.ToString(FormatUtils.FloatFormat, FormatUtils.Formatter)}" : $"{(Raw * 100).ToString(FormatUtils.FloatFormat, FormatUtils.Formatter)}%";
        }
        ...
        {
            internal const int SizeOf = Internal.Length.Calculated.SizeOf;
            ...
            {
                int hash = 13;
                hash = (hash * 7) + Percent.GetHashCode();
            ...
            public override string ToString()
            {
                return $"{{Val = {Value.ToString(FormatUtils.FloatFormat, FormatUtils.Formatter)}, {(Percent * 100).ToString(FormatUtils.FloatFormat, FormatUtils.Formatter)}%}}";
            }
        }
        #endregion
    }
