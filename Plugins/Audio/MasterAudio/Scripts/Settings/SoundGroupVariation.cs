...
namespace DarkTonic.MasterAudio {
    ...
    public class SoundGroupVariation : MonoBehaviour {
        ...
        public bool reReRandomizeOnLoop = false;
        ...
        private void Start() {
            ...
            SetOcclusion();
            SetFiltersForParameterCommands();
        ...
        public void SetBypassReverbZones() {
            var aBus = ParentGroup.BusForGroup;
            if (aBus == null) {
                return;
            }

            VarAudio.bypassReverbZones = aBus.bypassReverbZones;
        }
        ...
        private void SetFiltersForParameterCommands() {
            var hasLowHighFilterElement = MasterAudio.IsLowHighFilterUsedOnSoundGroupInParameterCommand(ParentGroup.GameObjectName);

            if (hasLowHighFilterElement.Item1) {
                if (UsesOcclusion) {
                    Debug.LogError("Sound Group: '" + ParentGroup.GameObjectName + "' uses Occlusion and at least one Parameter Command that affects the Low Pass Filter. These will cause a conflict.");
                } else {
            ...
            if (hasLowHighFilterElement.Item2) {
                if (HighPassFilter == null) {
                    _hpFilter = GetComponent<AudioHighPassFilter>();
                    if (_hpFilter == null) {
                        _hpFilter = gameObject.AddComponent<AudioHighPassFilter>();
                    }
                } else {
                    _hpFilter = GetComponent<AudioHighPassFilter>();
                }
        ...
        private void SetRandomVolume() {
            var varVol = OriginalVolume;

            if (useRandomVolume) {
                // random volume
                var randomVol = Random.Range(randomVolumeMin, randomVolumeMax);

                switch (randomVolumeMode) {
                    case RandomVolumeMode.AddToClipVolume:
                        varVol += randomVol;
                        break;
                    case RandomVolumeMode.IgnoreClipVolume:
                        varVol = OriginalVolume *  randomVol;
                        break;
                }
            }
        ...
        private void SetRandomPitch(float? pitch) {
            // compute pitch
            if (pitch.HasValue) {
        ...
        /// Never call this method. Used internally.
        /// </summary>
        /// <param name="groupCalcVolume"></param>
        public void SetPlaySoundGroupCalcVolume(float groupCalcVolume) {
            _playSndParam.GroupCalcVolume = groupCalcVolume;
        }
        ...
        public void UpdateTransformTracker(Transform sourceTrans) {
            ...
            if (sourceTrans.GetComponent<AudioTransformTracker>() == null) {
                if (sourceTrans.gameObject.activeInHierarchy) { // do not add if it's being destroyed
        ...
        public void SoundLoopStarted(int numberOfLoops) {
            if (reReRandomizeOnLoop) {
                if (useRandomPitch) {
                    SetRandomPitch(_playSndParam.Pitch);
                }
