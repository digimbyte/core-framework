...
#pragma warning disable 1591
        ...
        public const string MagicParameterDistanceFromListener = "[Listener Distance]";
        public const string MagicParameterFramesSinceStart = "[Frames Since Start]";
        public const string MagicParameterSecondsSinceStart = "[Seconds Since Start]";
        public const string MagicParameterSecondsSinceCommandStart = "[Seconds Since Command Invoked]";
        public const string MagicParameterTimesCommandInvoked = "[Times Command Invoked]";
        ...
        public bool addSteamAudioSources = false; 
        ...
        public bool logParameterCommands = false;
...
#region Private Variables
        ...
        private readonly List<RealTimeParameterCommandTracker> RealTimeParameterCommandTrackers = new List<RealTimeParameterCommandTracker>(4);
...
#region Master Audio enums
		public enum ParameterCommandDisplayMode {
            Simple,
            Advanced
        }
        ...
        public enum ParameterCmdCommand {
            None,
            InvokeParameterCommand,
            StopParameterCommandsOfTransform,
            StopAllOfParameterCommand
        }

        public enum RealTimeParameterCommand {
            None,
...
#region Inner classes & Structs
        ...
        private void Awake() {
            ...
            RealTimeParameterCommandTrackers.Clear();
        ...
        private void OnDrawGizmos() {
            ...
            if (SafeInstance == null || !Instance.showRangeSoundGizmos || !showParameterCommandGizmo || parameterCommandToGizmo == null) {
                return;
            }
            ...
            if (SafeInstance != null) {
                gizmoColor = Instance.rangeGizmoColor;
            }
        ...
        void OnDrawGizmosSelected() {
            ...
            var gizmoColor = Color.green;
            if (SafeInstance != null) {
                gizmoColor = Instance.selectedRangeGizmoColor;
            }
        ...
        private void ManualUpdate() {
            ...
            // This has to be done NOT in Update, because Low Pass Filter rolloff modifications won't work there (Unity bug since 2016).
            PerformRealTimeParameterCommandUpdates();
        }
        ...
        public static void UpdateRealTimeParameterCommandTrackersWithNewChainedVariation(SoundGroupVariation oldVariation, SoundGroupVariation newVariation) {
            for (var i = 0; i < Instance.RealTimeParameterCommandTrackers.Count; i++) {
                var tracker = Instance.RealTimeParameterCommandTrackers[i];
                if (!tracker.IsActive) {
                    continue;
                }

                // if no elements still playing, deactivate!
                for (var e = 0; e < tracker.ElementResults.Count; e++) {
                    var anElement = tracker.ElementResults[e];
                    if (anElement != null && anElement.ActingVariation != null) {
                        if (anElement.ActingVariation == oldVariation) {
                            anElement.ActingVariation = newVariation;
                            break; // only can be one tracker per variation. Get out!
                        }
                    }
                }
            }
        ...
        private void PerformRealTimeParameterCommandUpdates() {
            for (var i = 0; i < RealTimeParameterCommandTrackers.Count; i++) {
                var tracker = RealTimeParameterCommandTrackers[i];
                if (!tracker.IsActive) {
                    continue;
                }

                if (tracker.Actor == null) {
                    tracker.Reset();
                    continue;
                }

                if (tracker.ParameterCommand == null) {
                    tracker.Reset();
                    continue;
                }
                ...
                for (var e = 0; e < tracker.ElementResults.Count; e++) {
                    var anElement = tracker.ElementResults[e];
                    if (anElement != null && anElement.ActingVariation != null && anElement.ActingVariation.IsPlaying) {
                        hasActiveVoice = true;
                        break;
                    }
                }
                ...
                float paramValue = GetParamValue(tracker.Actor, selectedParam, tracker.ParameterCommand.ParameterName, tracker);

                if (tracker.ParameterCommand.StopCmdOutOfRange) {
                    var isOutsideValidRange = paramValue > tracker.ParameterCommand.ValidRangeMax || paramValue < tracker.ParameterCommand.ValidRangeMin;
                    if (isOutsideValidRange) {
                        if (!Instance.disableLogging && Instance.logParameterCommands) {
                            Debug.LogError("Parameter Command '" + tracker.ParameterCommand.CommandName + "' is stopping because Parameter '" + tracker.ParameterCommand.ParameterName
                                + "' is outside its valid range at: " + paramValue);
                        }
                ...
                for (var e = 0; e < tracker.ParameterCommand.Elements.Count; e++) {
                    var anElement = tracker.ParameterCommand.Elements[e];
                    if (anElement.SoundGroup == NoGroupName) {
                        continue;
                    }

                    var variationForElement = tracker.ElementResults[e].ActingVariation;

                    if (variationForElement == null) {
                        continue;
                    }

                    if (anElement.AffectVolume) {
                        var calculatedVolume = CalculateVolumeByParameterValue(paramValue, anElement);
                        variationForElement.AdjustVolume(calculatedVolume);
                    }

                    if (anElement.AffectPitch) {
                        var calculatedPitch = CalculatePitchByParameterValue(paramValue, anElement);
                        variationForElement.VarAudio.pitch = calculatedPitch;
                    }

                    if (anElement.AffectLowPassFilter) {
                        if (variationForElement.LowPassFilter != null) { // in case it somehow gets removed.
                            var cutoffFreq = CalculateLowPassCutoffFreqencyByParameterValue(paramValue, anElement);
                            // set the filter with initial value
                            variationForElement.LowPassFilter.cutoffFrequency = cutoffFreq;
                        }
                    }

                    if (anElement.AffectHighPassFilter) {
                        if (variationForElement.HighPassFilter != null) { // in case it somehow gets removed.
                            var cutoffFreq = CalculateHighPassCutoffFreqencyByParameterValue(paramValue, anElement);
                            // set the filter with initial value
                            variationForElement.HighPassFilter.cutoffFrequency = cutoffFreq;
                        }
                    }
                }
            }
                ...
                if (loggingEnabledForGrp || LogOutOfVoices) {
                    if (isNonSpecific) {
                        LogMessage("All " + sources.Count + " children of " + sType +
                                   " were busy. Will not play this sound for this instance. If you need more voices, increase the 'Voices / Weight' field on the Variation(s) in your Sound Group. A silently warming sound can also cause this.");
                    } 
                    else
                    { 
                        LogMessage("Child '" + randomSource.Variation.GameObjectName + "' of " + sType +
                                   " was busy. Will not play this sound for this instance. If you need more voices, increase the 'Voices / Weight' field on the Variation(s) in your Sound Group. A silently warming sound can also cause this.");
                    }
                }
        ...
        public static void DuckSoundGroup(string soundGroupName, AudioSource aSource, SoundGroupVariationUpdater actorUpdater) {
            ...
            if (ListenerTrans != null && aSource.spatialBlend > 0 && matchingDuck.enableDistanceDuckRatio) {
                var maxRange = aSource.maxDistance;
                var distanceFromListener = (ListenerTrans.position - aSource.transform.position).magnitude;

                switch (aSource.rolloffMode) {
                    case AudioRolloffMode.Custom:
                        var scaledDistance = distanceFromListener / maxRange;
                        var falloffCurve = aSource.GetCustomCurve(AudioSourceCurveType.CustomRolloff);
                        var effectiveVolumeOfSound = falloffCurve.Evaluate(scaledDistance);
                        duckVolumeCut *= effectiveVolumeOfSound;
                        break;
                    case AudioRolloffMode.Linear:
                    case AudioRolloffMode.Logarithmic: // way wrong for logarithmic but I need a formula. Don't understand this one: https://forum.unity.com/threads/how-to-calculate-audio-volume-based-on-logarithmic-rolloff-and-distance-from-listener.1527916/#post-9543136
        ...
        public static void StopAllSoundsOfTransform(Transform sourceTrans) {
            ...
            foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                var varList = audioGroupInfo.Sources;
...
#region Sound Group methods
        ...
        public static void PauseSoundGroup(string sType) {
            ...
            if (aGroup == null || aGroup.GameObjectName == VideoPlayerSoundGroupName || aGroup.IsPaused) { // can't pause again if already paused
                return;
            }
        ...
        public static void UnpauseSoundGroup(string sType) {
            ...
            if (aGroup == null || aGroup.GameObjectName == VideoPlayerSoundGroupName || !aGroup.IsPaused) { // can't unpause if not paused
                return;
            }
        ...
        public static Transform CreateSoundGroup(DynamicSoundGroup aGroup, EntityId? creatorInstanceId, bool errorOnExisting = true) {
            ...
            for (var i = 0; i < aGroup.groupVariations.Count; i++) {
                ...
                for (var j = 0; j < aVariation.weight; j++) {
                    ...
                    variation.reReRandomizeOnLoop = aVariation.reReRandomizeOnLoop;
                    ...
                    } else if (Instance.addSteamAudioSources && SteamAudioHelper.DarkTonicSteamAudioPackageInstalled()) {
        ...
        public static void SetGroupVolume(string sType, float volumeLevel) {
            ...
            for (var i = 0; i < theGroup.Sources.Count; i++) {
                ...
                float groupCalcVolume = aInfo.OriginalVolume * aInfo.LastPercentageVolume * aGroup.groupMasterVolume * busVolume * Instance._masterAudioVolume;
                var newVol = groupCalcVolume;

                if (aInfo.Variation.useRandomVolume) {
                    switch (aInfo.Variation.randomVolumeMode) {
                        case SoundGroupVariation.RandomVolumeMode.AddToClipVolume:
                            newVol += aInfo.LastRandomVolume;
                            break;
                        case SoundGroupVariation.RandomVolumeMode.IgnoreClipVolume:
                ...
                if (aInfo.Variation.IsPlaying) {
                    if (aInfo.Variation.VarAudio.loop && aInfo.Variation.reReRandomizeOnLoop) {
                        aInfo.Variation.SetPlaySoundGroupCalcVolume(groupCalcVolume);
                    }
                }
        ...
        private static void StopAllGradualProcesses() {
            // finish Occlusion
            for (var i = 0; i < Instance.VariationOcclusionFreqChanges.Count; i++) {
                var aFader = Instance.VariationOcclusionFreqChanges[i];
                aFader.CompletionTime = AudioUtil.Time;
            }

            // finish Bus fades
            for (var i = 0; i < Instance.BusFades.Count; i++) {
                BusFadeInfo aFader = Instance.BusFades[i];
                aFader.CompletionTime = AudioUtil.Time;
            }

            // finish Bus pitch glides
            for (var i = 0; i < Instance.BusPitchGlides.Count; i++) {
                BusPitchGlideInfo aGlider = Instance.BusPitchGlides[i];
                aGlider.CompletionTime = AudioUtil.Time;
            }

            // finish Group fades
            for (var i = 0; i < Instance.GroupFades.Count; i++) {
                var aFader = Instance.GroupFades[i];
                aFader.CompletionTime = AudioUtil.Time;
            }

            // finish Group pitch glides
            for (var i = 0; i < Instance.GroupPitchGlides.Count; i++) {
                GroupPitchGlideInfo aGlider = Instance.GroupPitchGlides[i];
                aGlider.CompletionTime = AudioUtil.Time;
            }
        }
...
#region Mixer methods
        ...
        public static void PauseMixer() {
            foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                PauseSoundGroup(audioGroupInfo.Group.GameObjectName);
            }
        }
        ...
        public static void UnpauseMixer() {
            foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                UnpauseSoundGroup(audioGroupInfo.Group.GameObjectName);
            }
        }
        ...
        /// </summary>
        public static void RestoreOriginalMixerVolumesAndPitches() {
            for (var i = 0; i < Instance.groupBuses.Count; i++) {
                var aBus = Instance.groupBuses[i];
                var bus = GrabBusByName(aBus.busName);
                SetBusVolume(bus, bus.OriginalVolume);
            }

            foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                foreach (var variation in audioGroupInfo.Sources) {
                    variation.Variation.VarAudio.volume = variation.Variation.original_volume;
                    variation.Variation.VarAudio.pitch = variation.Variation.original_pitch;
                }
                RefillSoundGroupPool(audioGroupInfo.Group.name);
            }
        }
...
#region Bus methods
        ...
        public static void PauseBus(string busName) {
            ...
            if (bus == null || bus.IsPaused) {
                return; // can't pause again if already paused
            }
        ...
        public static void UnpauseBus(string busName) {
            ...
            var bus = GrabBusByName(busName);
            if (bus == null || !bus.IsPaused) {
                return; // can't unpause again if not paused
            }
            ...
            for (var n = 0; n < RuntimeSoundGroupNames.Count; n++) {
                ...
                if (aGroup.busIndex == busIndex) {
                    ...
                    for (var i = 0; i < groupInfo.Sources.Count; i++)
                    {
                        ...
                        aVariation.SetBypassReverbZones();
                    }
        ...
        private static void SetBusVolume(GroupBus bus, float newVolume, bool fromBusDucking = false) {
            ...
            foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                RecalculateGroupVolumes(audioGroupInfo, bus);
            }
        }
...
#region Custom Events
        ...
        public static void FireCustomEvent(string customEventName, Transform originObject, bool logDupe = true) {
            ...
            if (!Instance.disableLogging && Instance.logCustomEvents) {
                Debug.Log("Firing Custom Event: " + customEventName + " from actor '" + originObject.name + "'.");
            }
        ...
        /// <param name="isTemporary">true if created by DynamicSoundGroupCreator</param>
        /// <param name="errorOnDuplicate"></param>
        public static void CreateRealTimeParameter(string paramName, float initialValue, bool isExpanded, float minValue, float maxValue, 
            bool isTemporary = false, bool errorOnDuplicate = true) {

            if (Application.isPlaying && AppIsShuttingDown) {
                return;
            }

            RealTimeParameter matchingParam = null;

            for (var i = 0; i < Instance.realTimeParameters.Count; i++) {
                var aParam = Instance.realTimeParameters[i];
                if (aParam.ParameterName == paramName) {
                    matchingParam = aParam;
                    break;
                }
            }
            ...
            if (matchingParam != null) {
                if (errorOnDuplicate) {
                    Debug.LogError("You already have a Parameter named '" + paramName + "'. No need to add it again.");
                    return;
                }
            }

            Instance.realTimeParameters.Add(new RealTimeParameter {
                ParameterName = paramName,
                Value = initialValue,
                IsEditing = false,
                IsExpanded = isExpanded,
                ProspectiveName = paramName,
                MinValue = minValue,
                MaxValue = maxValue,
                IsTemporary = isTemporary
            });
        ...
        /// <param name="errorOnDuplicate"></param>
        /// <returns></returns>
        public static ParameterCommand CreateRealTimeParameterCommand(string commandName, string paramName, bool isExpanded,
            float minDisplayCurveX,
            float maxDisplayCurveX,
            ...
            ParameterCommand matchingCmd = null;

            for (var i = 0; i < Instance.parameterCommands.Count; i++) {
                var aParam = Instance.parameterCommands[i];
                if (aParam.CommandName == commandName) {
                    matchingCmd = aParam;
                    break;
                }
            }
            ...
            if (matchingCmd != null) {
                if (errorOnDuplicate) {
                    Debug.LogError("You already have a Parameter Command named '" + commandName + "'. No need to add it again.");
                    return null;
                }
            }

            var newCmd = new ParameterCommand {
                CommandName = commandName,
                IsEditing = false,
                IsExpanded = isExpanded,
                ProspectiveName = commandName,
                ParameterName = paramName,
                IsTemporary = isTemporary,
                MinDisplayCurveX = minDisplayCurveX,
                MaxDisplayCurveX = maxDisplayCurveX,
                StopCmdOutOfRange = false                
            };

            Instance.parameterCommands.Add(newCmd);
        ...
        /// <param name="variationMode"></param>
        /// <param name="soundGroup"></param>
        /// <param name="volume"></param>
        /// <param name="overridePitch"></param>
        /// <param name="pitch"></param>
        /// <param name="delaySound"></param>
        /// <param name="fadeInRangeStart"></param>
        /// <param name="fadeInRangeEnd"></param>
        /// <param name="fadeOutRangeStart"></param>
        /// <param name="fadeOutRangeEnd"></param>
        /// <param name="soundSpawnLocationMode"></param>
        /// <param name="affectVolume"></param>
        /// <param name="volumeAnimationCurve"></param>
        /// <param name="affectPitch"></param>
        /// <param name="pitchAnimationCurve"></param>
        /// <param name="showFadeInOutFields"></param>
        /// <param name="maxDisplayVolumeCurveY"></param>
        /// <param name="errorOnDuplicate"></param>        
        public static void CreateParameterCommandElement(ParameterCommand cmd, string elementName, string variationName, bool isExpanded,
            bool showAdvanced, EventSounds.VariationType variationMode, string soundGroup, float volume, bool overridePitch, float pitch,
            float delaySound, float fadeInRangeStart, float fadeInRangeEnd, float fadeOutRangeStart, float fadeOutRangeEnd,
            SoundSpawnLocationMode soundSpawnLocationMode,
            bool showCurves,
            bool affectVolume, AnimationCurve volumeAnimationCurve,
            bool affectPitch, AnimationCurve pitchAnimationCurve, bool showFadeInOutFields, 
            bool affectLowPassFilter, AnimationCurve lowPassAnimationCurve,
            bool affectHighPassFilter, AnimationCurve highPassAnimationCurve,
            float maxDisplayVolumeCurveY = 1f,
            bool errorOnDuplicate = true) {
            ...
            ParameterCommandElement matchingElement = null;

            for (var i = 0; i < cmd.Elements.Count; i++) {
                var anElement = cmd.Elements[i];
                if (anElement.ElementName == elementName) {
                    matchingElement = anElement;
                    break;
                }
            }
            ...
            if (matchingElement != null) {
                if (errorOnDuplicate) {
                    Debug.LogError("Parameter Command '" + cmd.CommandName + "' already has an element named '" + elementName + "'. No need to add it again.");
                    return;
                }
            }

            cmd.Elements.Add(new ParameterCommandElement {
                ...
                FadeInRangeStart = fadeInRangeStart,
                FadeInRangeEnd = fadeInRangeEnd,
                FadeOutRangeStart = fadeOutRangeStart,
                FadeOutRangeEnd = fadeOutRangeEnd,
                SoundSpawnLocationMode = soundSpawnLocationMode,
                ShowCurves = showCurves,
                AffectVolume = affectVolume,
                VolumeAnimationCurve = volumeAnimationCurve,
                AffectPitch = affectPitch,
                PitchAnimationCurve = pitchAnimationCurve,
                ShowFadeInOutFields = showFadeInOutFields,
                MaxDisplayVolumeCurveY = maxDisplayVolumeCurveY,
                AffectLowPassFilter = affectLowPassFilter,
                LowPassAnimationCurve = lowPassAnimationCurve,
                AffectHighPassFilter = affectHighPassFilter,
                HighPassAnimationCurve = highPassAnimationCurve
            });
        }
        ...
        public static void UpdateRealTimeParameter(string parameterName, float value) {
            if (string.IsNullOrWhiteSpace(parameterName)) {
                Debug.LogError("Cannot update Parameter name when the name is empty.");
                return;
            }

            var param = GetRealTimeParameterByName(parameterName);
            if (param == null) {
                Debug.LogError("Parameter '" + parameterName + "' could not be found in Master Audio.");
                return;
            }

            // don't go beyond the min/max
        ...
        public static void StopParameterCommandsByCommandName(string commandName) {
            if (AppIsShuttingDown) {
                return;
            }
            ...
            for (var i = 0; i < Instance.RealTimeParameterCommandTrackers.Count; i++) {
                var aTracker = Instance.RealTimeParameterCommandTrackers[i];
                if (!aTracker.IsActive) {
                    continue;
                }

                if (aTracker.ParameterCommand == null || aTracker.ParameterCommand.CommandName != commandName) {
                    continue;
                }

                for (var e = 0; e < aTracker.ElementResults.Count; e++) {
                    var element = aTracker.ElementResults[e];
                    if (element.ActingVariation == null || !element.ActingVariation.IsPlaying) {
                        continue;
                    }

                    element.ActingVariation.Stop();
                }
        ...
        /// </summary>
        /// <param name="commandName">Command name</param>
        /// <param name="originObject">Actor that made the sounds</param>
        public static void StopParameterCommandsOfTransform(string commandName, Transform originObject) {
            if (AppIsShuttingDown) {
                return;
            }
            ...
            for (var i = 0; i < Instance.RealTimeParameterCommandTrackers.Count; i++) {
                ...
                for (var e = 0; e < aTracker.ElementResults.Count; e++) {
                    ...
                    if (!element.ActingVariation.WasTriggeredFromTransform(originObject)) {
                        continue; // should always either be true or false for all of the sound Tracker
                    }
        ...
        public static void StopParameterCommandByInstanceId(Guid? instanceId) {
            if (AppIsShuttingDown) {
                return;
            }
            ...
            for (var i =0; i < Instance.RealTimeParameterCommandTrackers.Count; i++) {
                ...
                if (aTracker.InstanceId != instanceId) {
                    continue;
                }
        ...
        /// </summary>
        /// <param name="parameterCommandName">Parameter Command to fire</param>
        /// <param name="originObject">The actor making the sounds</param>
        public static Guid? InvokeParameterCommand(string parameterCommandName, Transform originObject) {
            if (AppIsShuttingDown) {
                return null;
            }

            if (parameterCommandName == NoGroupName || parameterCommandName == DynamicGroupName) {
                return null;
            }

            if (originObject == null) {
                Debug.LogError("Parameter Command '" + parameterCommandName + "' cannot be invoked without an originObject passed in.");
                return null;
            }

            ParameterCommand paramCmd = GetParameterCmdByName(parameterCommandName);

            if (paramCmd == null) {
                Debug.LogError("Parameter Command '" + parameterCommandName + "' was not found in Master Audio.");
                return null;
            }

            if (paramCmd.Elements.Count == 0) {
                Debug.LogError("Parameter Command '" + parameterCommandName + "' has 0 elements. Aborting Command.");
                return null;
            }

            var hasInvalidElement = false;
            for (var i = 0; i < paramCmd.Elements.Count; i++) {
                var element = paramCmd.Elements[i];
                if (element.SoundGroup == NoGroupName) {
                    Debug.LogError("Parameter Command '" + parameterCommandName + "', element '" + element.ElementName + "' has no Sound Group selected. Aborting Command.");
            ...
            if (!Instance.disableLogging && Instance.logParameterCommands) {
                Debug.Log("Firing Parameter Command: " + parameterCommandName + " from actor '" + originObject.name + "'.");
            }

            float paramValue = GetParamValue(originObject, selectedParam, paramCmd.ParameterName, null);

            
            if (paramCmd.StopCmdOutOfRange) {
                var isOutsideValidRange = paramValue > paramCmd.ValidRangeMax || paramValue < paramCmd.ValidRangeMin;
                if (isOutsideValidRange) {
                    if (!Instance.disableLogging && Instance.logParameterCommands) {
                        Debug.LogError("Parameter Command '" + paramCmd.CommandName + "' is not invoking because Parameter '" + paramCmd.ParameterName
                            + "' is outside its valid range at: " + paramValue);
                    }
            ...
            //check for an inactive Param Cmd Tracker to use
            RealTimeParameterCommandTracker tracker = null;
            for (var t = 0; t < Instance.RealTimeParameterCommandTrackers.Count; t++) {
                var aTracker = Instance.RealTimeParameterCommandTrackers[t];
                if (!aTracker.IsActive) {
                    tracker = aTracker;
                    break;
                }
            ...
            // otherwise, create a new one
            if (tracker == null) {
                tracker = new RealTimeParameterCommandTracker {
                    IsActive = false
                };
                Instance.RealTimeParameterCommandTrackers.Add(tracker);
            }

            tracker.ElementResults.Clear();

            var elementsPlayed = 0;

            for (var i = 0; i < paramCmd.Elements.Count; i++) {
                var anElement = paramCmd.Elements[i];

                float computedVolume;
                
                if (anElement.AffectVolume) {
                    computedVolume = CalculateVolumeByParameterValue(paramValue, anElement);
                } else {
                    computedVolume = anElement.Volume;
                }

                float? computedPitch = null;
                if (anElement.AffectPitch) {
                    computedPitch = CalculatePitchByParameterValue(paramValue, anElement);                    
                } else {
                    if (anElement.OverridePitch) {
                        computedPitch = anElement.Pitch;
                    }
                }

                string variationName = null;
                if (anElement.VariationMode == EventSounds.VariationType.PlaySpecific) {
                    variationName = anElement.VariationName;
                }

                PlaySoundResult playSoundResult = null;

                switch (anElement.SoundSpawnLocationMode) {
                    case SoundSpawnLocationMode.AttachToCaller:
                        playSoundResult = PlaySound3DFollowTransform(anElement.SoundGroup, originObject, computedVolume, computedPitch, anElement.DelaySound, variationName);
                        break;
                    case SoundSpawnLocationMode.CallerLocation:
                        playSoundResult = PlaySound3DAtTransform(anElement.SoundGroup, originObject, computedVolume, computedPitch, anElement.DelaySound, variationName);
                        break;
                }
                ...
                if (playSoundResult.ActingVariation != null) {
                    ...
                    if (anElement.AffectLowPassFilter) {
                        var initialCutoffFreq = CalculateLowPassCutoffFreqencyByParameterValue(paramValue, anElement);
                        // set the filter with initial value
                        playSoundResult.ActingVariation.LowPassFilter.cutoffFrequency = initialCutoffFreq;
                    }

                    if (anElement.AffectHighPassFilter) {
                        var initialCutoffFreq = CalculateHighPassCutoffFreqencyByParameterValue(paramValue, anElement);
                        // set the filter with initial value
                        playSoundResult.ActingVariation.HighPassFilter.cutoffFrequency = initialCutoffFreq;
                    }
                }

                tracker.ElementResults.Add(playSoundResult);
            }

            if (elementsPlayed == 0) {
                if (!Instance.disableLogging && Instance.logParameterCommands) {
                    Debug.Log("Parameter Command '" + parameterCommandName + " could not play any of its elements. Command aborted.");
                }
                return null; 
            }

            var instanceId = tracker.ActivateAndGetInstanceId(paramCmd, originObject);
            return instanceId;
        }
...
/*! \endcond */
        ...
        private static float CalculateLowPassCutoffFreqencyByParameterValue(float paramValue, ParameterCommandElement anElement) {
            var computedLowPassRolloffFrequence = anElement.LowPassAnimationCurve.Evaluate(paramValue);
            return computedLowPassRolloffFrequence;
        }

        private static float CalculateHighPassCutoffFreqencyByParameterValue(float paramValue, ParameterCommandElement anElement) {
            var computedHighPassRolloffFrequence = anElement.HighPassAnimationCurve.Evaluate(paramValue);
            return computedHighPassRolloffFrequence;
        }

        private static float CalculateVolumeByParameterValue(float paramValue, ParameterCommandElement anElement) {
            var computedVolume = anElement.VolumeAnimationCurve.Evaluate(paramValue);
            return computedVolume;
        }

        private static float CalculatePitchByParameterValue(float paramValue, ParameterCommandElement anElement) {
            var computedPitch = anElement.PitchAnimationCurve.Evaluate(paramValue);
            return computedPitch;
        }

        private static ParameterCommand GetParameterCmdByName(string parameterCommand) {
            ParameterCommand paramCmd = null;
            for (var i = 0; i < Instance.parameterCommands.Count; i++) {
                var anEvent = Instance.parameterCommands[i];
                if (anEvent.CommandName == parameterCommand) {
                    paramCmd = anEvent;
                    break;
                }
            }
        ...
        private static float GetParamValue(Transform originObject, RealTimeParameter selectedParam, string paramName, RealTimeParameterCommandTracker tracker) {
            float paramValue;

            switch (paramName) {
                case MagicParameterFramesSinceStart:
                    paramValue = Time.frameCount;
                    break;
                case MagicParameterSecondsSinceStart:
                    paramValue = Time.realtimeSinceStartup;
                    break;
                case MagicParameterDistanceFromListener:
                    paramValue = (ListenerTrans.position - originObject.position).magnitude;
                    break;
                case MagicParameterSecondsSinceCommandStart:
                    if (tracker != null) {
                        paramValue = Time.realtimeSinceStartup - tracker.TimeInvoked.Value;
                    } else {
                        paramValue = 0f;
                    }
                    break;
                case MagicParameterTimesCommandInvoked:
                    if (tracker != null && tracker.ParameterCommand != null) {
                        paramValue = tracker.ParameterCommand.NumberOfTimesInvoked;
                    } else {
                        paramValue = 0f;
                    }
                    break;
        ...
        public static bool MixerMuted {
            ...
            set {
                ...
                if (value) {
                    foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                        MuteGroup(audioGroupInfo.Group.GameObjectName, false);
                    }
                } else {
                    foreach (var audioGroupInfo in Instance.AudioSourcesBySoundType.Values) {
                        UnmuteGroup(audioGroupInfo.Group.GameObjectName, false);
                    }
                }
        ...
        public List<string> ParameterNames {
            get {
                var paramNames = ParameterHardCodedNames;

                var parameters = Instance.realTimeParameters;

                // ReSharper disable once ForCanBeConvertedToForeach
                for (var i = 0; i < parameters.Count; i++) {
                    paramNames.Add(parameters[i].ParameterName);
                }
        ...
        /// </summary>
        public List<string> ParameterCommandNames {
            get {
                var paramCmdNames = ParameterCommandHardCodedNames;

                var paramCommands = Instance.parameterCommands;

                // ReSharper disable once ForCanBeConvertedToForeach
                for (var i = 0; i < paramCommands.Count; i++) {
                    paramCmdNames.Add(paramCommands[i].CommandName);
                }

                return paramCmdNames;
            }
        }
        ...
        /// Only used internally, do not use this property
        /// </summary>
        public static List<string> ParameterHardCodedNames {
            get {
                return new List<string> {
                    DynamicGroupName,
                    NoGroupName,
                    MagicParameterDistanceFromListener,
                    MagicParameterFramesSinceStart,
                    MagicParameterSecondsSinceStart,
                    MagicParameterSecondsSinceCommandStart,
                    MagicParameterTimesCommandInvoked
                };
            }
        }
        ...
        /// Only used internally, do not use this property
        /// </summary>
        public static List<string> ParameterCommandHardCodedNames {
            get {
                return new List<string> {
                    DynamicGroupName,
                    NoGroupName
        ...
        public AudioSource GetNamedOrFirstAudioSource(ParameterCommandElement parameterCommandElement) {
            ...
            var grp = Instance.transform.Find(parameterCommandElement.SoundGroup);
            if (grp == null) {
                parameterCommandElement.ColliderMaxDistance = 0;
                return null;
            }
        ...
        public List<AudioSource> GetAllVariationAudioSources(ParameterCommandElement element) {
            ...
            var grp = Instance.transform.Find(element.SoundGroup);
            if (grp == null) {
                element.ColliderMaxDistance = 0;
                return null;
            }
        ...
        /// <summary>
        /// Used by SoundGroupVariation to set up initial filter components. Do not call.
        /// </summary>
        /// <param name="soundType"></param>
        /// <returns></returns>
        public static Tuple<bool, bool> IsLowHighFilterUsedOnSoundGroupInParameterCommand(string soundType) {
            if (SafeInstance == null) {
                return new Tuple<bool, bool>(false, false);
            }

            var hasLowPassElement = false;
            var hasHighPassElement = false;

            for (var i = 0; i < Instance.parameterCommands.Count; i++) {
                var command = Instance.parameterCommands[i];

                for (var j = 0; j < command.Elements.Count; j++) {
                    var element = command.Elements[j];
                    if (!element.SoundGroup.Equals(soundType, StringComparison.InvariantCulture)) {
                        continue;
                    }

                    if (element.AffectLowPassFilter) {
                        hasLowPassElement = true;
                    }
                    if (element.AffectHighPassFilter) {
                        hasHighPassElement = true;
                    }
                }
            }

            return new Tuple<bool, bool>(hasLowPassElement, hasHighPassElement);
        }

        /// <summary>
        /// Grab the Real Time Parameter that a Parameter Command uses
        /// </summary>
        /// <param name="paramCmd">The Parameter Command in question</param>
        /// <returns>RealTimeParameter</returns>
        public static RealTimeParameter GetRealTimeParameterFromCommand(ParameterCommand paramCmd) {
            return GetRealTimeParameterByName(paramCmd.ParameterName);
        }

        /// <summary>
        /// Grab a Real Time Parameter Out of the list, by name.
        /// </summary>
        /// <param name="paramName">Parameter Name</param>
        /// <returns>RealTimeParameter</returns>
        public static RealTimeParameter GetRealTimeParameterByName(string paramName) {
        ...
        /// <summary>
        /// Used by the Inspector to display active instances
        /// </summary>
        /// <param name="cmd">ParameterCommand</param>
        /// <returns>int</returns>
        public static int GetCommandTrackersOfCommandType(ParameterCommand cmd) {
            var matchCount = 0;
            ...
            for (var i = 0; i < Instance.RealTimeParameterCommandTrackers.Count; i++) {
                ...
                if (aTracker.ParameterCommand == cmd) {
                    matchCount++;
                }
            }
