   ...
   public abstract class ConfigBase : EditorWindow
   {
      ...
      protected void showConfiguration()
      {
         ...
         scrollPosConfig = EditorGUILayout.BeginScrollView(scrollPosConfig, false, false);
         {
            ...
            {
               var httpAllowed = UnityEditor.PlayerSettings.insecureHttpOption == UnityEditor.InsecureHttpOption.AlwaysAllowed;
               httpAllowed = EditorGUILayout.Toggle(new GUIContent("HTTP Download", "Allow plain HTTP for streaming/downloads (Player Settings > Insecure Http: Always Allowed when on; Not Allowed when off)."), httpAllowed);
               UnityEditor.PlayerSettings.insecureHttpOption = httpAllowed ? UnityEditor.InsecureHttpOption.AlwaysAllowed : UnityEditor.InsecureHttpOption.NotAllowed;
            }
         }
