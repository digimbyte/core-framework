...
namespace DarkTonic.MasterAudio.EditorScripts {
    ...
    public class MasterAudioInspector : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            if (!isProjectView && !Application.isPlaying && _sounds.gameObject.layer != 2) { // "ignore raycast layer"
            ...
            var cont = new GUIContent("Collapse All Sections", "Click to collapse all sections");
            if (GUILayout.Button(cont, EditorStyles.toolbarButton, GUILayout.Width(140))) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "Collapse all sections");
                _sounds.showAdvancedSettings = false;
                _sounds.areGroupsExpanded = false;
                _sounds.playListExpanded = false;
                _sounds.showMusicDucking = false;
                _sounds.showOcclusion = false;
                _sounds.showLocalization = false;
                _sounds.showCustomEvents = false;
                _sounds.showVideoPlayerSettings = false;
                _sounds.showRTPC = false;
            }
            ...
            var state = _sounds.showAdvancedSettings;
            var text = "Advanced Settings";
            ...
            if (_sounds.showAdvancedSettings) {
                ...
                if (_sounds.logAdvancedExpanded) {
                    ...
                    if (!_sounds.disableLogging) {
                        ...
                        newLog = EditorGUILayout.Toggle("Log Parameter Commands", _sounds.logParameterCommands);
                        if (newLog != _sounds.logParameterCommands) {
                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Log Parameter Commands");
                            _sounds.logParameterCommands = newLog;
                        }
                    } else {
                ...
                if (_sounds.visualAdvancedExpanded) {
                    ...
                    var newGiz2 = EditorGUILayout.Toggle(new GUIContent("Show All Range Gizmos", "Turning this option on will show you the max distance of your selected Sound Group in Ambient Sound and Event Sounds scripts (Real-Time Parameter Commands too) so you can see how far the range is."), _sounds.showRangeSoundGizmos);
                ...
                if (_sounds.vrSettingsExpanded) {
                    ...
                    if (!SpatializerHelper.IsSupportedSpatializer) {
                        DTGUIHelper.ShowLargeBarAlert("You must select a supported Spatializer Plugin on the Audio Settings dialog before settings here will have any effect. Oculus, Meta XR, Resonance Audio and Steam Audio are currently supported.");
                    }

                    var newSpatialize = EditorGUILayout.Toggle(new GUIContent("Use Spatializer", "Turn this on if you have selected a supported Spatializer Plugin on the AudioManager settings screen. All Sound Group Audio Sources will automatically turn on 'Spatialize'."), _sounds.useSpatializer);
                    ...
                    if (SpatializerHelper.IsResonanceAudioSpatializer) {
                        ...
                        GUI.contentColor = Color.white;
                    } else if (SpatializerHelper.IsMetaXrSpatializer) {
                        GUI.contentColor = DTGUIHelper.BrightButtonColor;

                        var newOculus = EditorGUILayout.Toggle(new GUIContent("Add Meta XR Audio Sources", "This will immediately add a Meta XR Audio Source component to every Variation in every Sound Group, and also add one to any Sound Groups created at runtime (from Dynamic Sound Group Creators)"), _sounds.addOculusAudioSources);
                        if (newOculus != _sounds.addOculusAudioSources) {
                            if (!OculusAudioHelper.DarkTonicOculusAudioPackageInstalled()) {
                                DTGUIHelper.ShowAlert("Install the optional package 'MA_OculusMetaXR' to get this to work properly.");
                            } else {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Add Meta XR Audio Sources");
                                _sounds.addOculusAudioSources = newOculus;
                        ...
                        GUI.contentColor = Color.white;
                    } else if (SpatializerHelper.IsSteamAudioSpatializer) {
                        if (_sounds.useSpatializer) {
                            var newwPost = EditorGUILayout.Toggle(new GUIContent("Spatialize Post FX", "Turn this on to Spatialize Post Effects (Steam Audio option)."), _sounds.useSpatializerPostFX);
                            if (newwPost != _sounds.useSpatializerPostFX) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Spatialize Post FX");
                                _sounds.useSpatializerPostFX = newwPost;
                            }
                        ...
                        var newRes = EditorGUILayout.Toggle(new GUIContent("Add Steam Audio Sources", "This will immediately add a Steam Audio Source component to every Variation in every Sound Group, and also add one to any Sound Groups created at runtime (from Dynamic Sound Group Creators)"), _sounds.addSteamAudioSources);
                        if (newRes != _sounds.addSteamAudioSources) {
                            if (!SteamAudioHelper.DarkTonicSteamAudioPackageInstalled()) {
                                DTGUIHelper.ShowAlert("Install the optional package 'MA_SteamAudio' to get this to work properly.");
                            } else {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Add Steam Audio Sources");
                                _sounds.addSteamAudioSources = newRes;

                                if (newRes) {
                                    SteamAudioHelper.AddSteamAudioSourceToAllVariations();
                                } else {
                                    SteamAudioHelper.RemoveSteamAudioSourceFromAllVariations();
                                }
                            }
                        }
            ...
            if (_sounds.areGroupsExpanded) {
                ...
                if (groups.Count == 0) {
                    ...
                    for (var l = 0; l < filteredGroups.Count; l++) {
                        ...
                        switch (groupButtonPressed) {
                            case DTGUIHelper.DTFunctionButtons.Pause:
                                MasterAudio.PauseSoundGroup(aGroup.GameObjectName);
                                break;
                            case DTGUIHelper.DTFunctionButtons.Unpause:
                                MasterAudio.UnpauseSoundGroup(aGroup.GameObjectName);
                                break;
                ...
                if (_sounds.groupBuses.Count > 0) {
                    ...
                    for (var i = 0; i < _sounds.groupBuses.Count; i++) {
                        ...
                        GUILayout.Space(10);
                        var newBypassReverbZones = GUILayout.Toggle(aBus.bypassReverbZones, "Bypass Reverb Zones");
                        if (newBypassReverbZones != aBus.bypassReverbZones) {
                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Bypass Reverb Zones");
                            aBus.bypassReverbZones = newBypassReverbZones;
                        }
                    ...
                    if (busToPause.HasValue) {
                        MasterAudio.PauseBus(_sounds.groupBuses[busToPause.Value].busName);
                    }
                    if (busToUnpause.HasValue) {
                        MasterAudio.UnpauseBus(_sounds.groupBuses[busToUnpause.Value].busName);
                    }
            ...
            if (_sounds.playListExpanded) {
                ...
                EditorGUI.indentLevel = 0;  // Space will handle this for the header
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Playlist Controller Setup", EditorStyles.miniBoldLabel, GUILayout.Width(135));
            ...
            if (state != _sounds.showMusicDucking) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Show Ducking");
            ...
            if (_sounds.showMusicDucking) {
                ...
                if (_sounds.musicDuckingSounds.Count == 0) {
                    ...
                    for (var i = 0; i < _sounds.musicDuckingSounds.Count; i++) {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        EditorGUI.indentLevel = 1;
                        var exp = DTGUIHelper.Foldout(duckSound.isExpanded, "Duck Sound");
                        ...
                        if (duckSound.isExpanded) {
                            ...
                            if (duckSound.enableDistanceDuckRatio) {
                                DTGUIHelper.ShowColorWarning("Logarithmic rolloff percentage cannot be calculated accurately. Try and use a Custom rolloff if you aren't already.");
                            }
            ...
            if (isExp != _sounds.showOcclusion) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Occlusion");
                _sounds.showOcclusion = isExp;
            }
            ...
            state = _sounds.showLocalization;
            text = "Languages";
            ...
            if (_sounds.showCustomEvents) {
                            ...
                            if (!anEvent.IsEditing || Application.isPlaying) {
                                ...
                                switch (buttonPressed) {
                                    case DTGUIHelper.DTFunctionButtons.Cancel:
                                        ...
                                        anEvent.ProspectiveName = anEvent.EventName;
                                        _isDirty = true;
                                        break;
                                    case DTGUIHelper.DTFunctionButtons.Save:
                ...
                if (eventEditing != null) {
                    ...
                    for (var c = 0; c < _sounds.customEvents.Count; c++) {
                        ...
                        if (eventEditing == evt) {
                            evt.IsEditing = true;
                            evt.ProspectiveName = evt.EventName;
                        } else {
                            evt.IsEditing = false;
                        }
            ...
            state = _sounds.showRTPC;
            text = "Real-Time Parameter Commands";

            DTGUIHelper.ShowCollapsibleSection(ref state, text);

            GUILayout.Space(2f);

            if (state != _sounds.showRTPC) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Real-Time Parameter Commands");
                _sounds.showRTPC = state;
            }

            GUI.color = Color.white;

            DTGUIHelper.AddHelpIconNoStyle("https://www.dtdevtools.com/docs/masteraudio/RealTimeParameterCommands.htm");
            ...
            if (_sounds.showRTPC) {
                ...
                var labelName = "Parameter Management";
                if (!_sounds.showParameterManagement) {
                    labelName += " (" + _sounds.realTimeParameters.Count + ")";
                }

                var newShow = DTGUIHelper.Foldout(_sounds.showParameterManagement, labelName);
                if (newShow != _sounds.showParameterManagement) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Parameter Management");
                    _sounds.showParameterManagement = !_sounds.showParameterManagement;
                }
                DTGUIHelper.AddHelpIconNoStyle("https://www.dtdevtools.com/docs/masteraudio/RealTimeParameterCommands.htm#ParameterManagement");
                EditorGUILayout.EndHorizontal();
                ...
                if (_sounds.showParameterManagement) {
                    ...
                    if (GUILayout.Button(new GUIContent("Add Parameter"), EditorStyles.toolbarButton, GUILayout.Width(100))) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "Add Parameter");
                        var initialName = ("NewParameter " + (_sounds.realTimeParameters.Count + 1));
                        MasterAudio.CreateRealTimeParameter(initialName, 0f, true, 0f, 1000f, false, false);
                    }

                    if (_sounds.realTimeParameters.Count > 0) {
                        var hasExpanded = false;
                        foreach (var t in _sounds.realTimeParameters) {
                            if (!t.IsExpanded) {
                                continue;
                            }
                        ...
                        if (GUILayout.Button(new GUIContent(theButtonText), EditorStyles.toolbarButton, GUILayout.Width(100))) {
                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "Expand / Collapse All");
                            var newExpandState = hasExpanded ? false : true;

                            foreach (var t in _sounds.realTimeParameters) {
                                t.IsExpanded = newExpandState;
                            }
                        }
                    }
                    ...
                    GUI.contentColor = Color.white;
                    int? parameterToRemove = null;

                    if (_sounds.realTimeParameters.Count == 0) {
                        DTGUIHelper.ShowLargeBarAlert("You currently have no Parameters set up.");
                    }

                    RealTimeParameter parameterEditing = null;
                    RealTimeParameter parameterRenaming = null;

                    for (var i = 0; i < _sounds.realTimeParameters.Count; i++) {
                        var param = _sounds.realTimeParameters[i];

                        DTGUIHelper.StartGroupHeader(1);

                        EditorGUILayout.BeginHorizontal();

                        if (!param.IsEditing || Application.isPlaying) {
                            EditorGUI.indentLevel = 1;
                            var exp = DTGUIHelper.Foldout(param.IsExpanded, param.ParameterName);
                            if (exp != param.IsExpanded) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle expand Parameter");
                                param.IsExpanded = exp;
                            }

                            if (!Application.isPlaying) {
                                var settingsIcon = new GUIContent(MasterAudioInspectorResources.GearTexture, "Click to edit Parameter Name");
                                if (GUILayout.Button(settingsIcon, EditorStyles.toolbarButton, GUILayout.Width(24), GUILayout.Height(16))) {
                                    parameterEditing = param;
                                }
                            } else {
                                if (!param.IsExpanded) {
                                    var oldColor = GUI.contentColor;
                                    GUI.contentColor = Color.yellow;
                                    EditorGUILayout.LabelField("Value: " + param.Value, GUILayout.Width(130));
                            ...
                            if (!Application.isPlaying) {
                                if (DTGUIHelper.AddDeleteIcon("Parameter")) {
                                    parameterToRemove = i;
                                }
                            }
                        } else {
                            EditorGUI.indentLevel = 0;
                            GUI.backgroundColor = DTGUIHelper.BrightTextColor;
                            var tex = EditorGUILayout.TextField("", param.ProspectiveName);
                            if (tex != param.ProspectiveName) {
                                param.ProspectiveName = tex;
                                _isDirty = true;
                            }
                        ...
                        if (param.IsExpanded) {
                            if (Application.isPlaying) {
                                EditorGUILayout.BeginHorizontal();
                                var currentVal = EditorGUILayout.FloatField("Current Value", param.Value);
                                if (currentVal != param.Value) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Current Value");
                            ...
                            var newMin = EditorGUILayout.FloatField("Min Value", param.MinValue);
                            if (newMin != param.MinValue) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Min Value");
                                param.MinValue = newMin;
                            }
                            ...
                            var newMax = EditorGUILayout.FloatField("Max Value", param.MaxValue);
                            if (newMax != param.MaxValue) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Max Value");
                                param.MaxValue = newMax;
                            }
                    ...
                    if (parameterToRemove.HasValue) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "delete Parameter");
                        _sounds.realTimeParameters.RemoveAt(parameterToRemove.Value);
                    }

                    if (parameterRenaming != null) {
                        var isValidName = true;

                        if (string.IsNullOrEmpty(parameterRenaming.ProspectiveName)) {
                            isValidName = false;
                            DTGUIHelper.ShowAlert("You cannot have a blank Parameter name.");
                        }

                        if (isValidName) {
                            // ReSharper disable once ForCanBeConvertedToForeach
                            for (var c = 0; c < _sounds.realTimeParameters.Count; c++) {
                                var param = _sounds.realTimeParameters[c];
                                // ReSharper disable once InvertIf
                                if (param != parameterRenaming && param.ParameterName == parameterRenaming.ProspectiveName) {
                                    isValidName = false;
                                    DTGUIHelper.ShowAlert("You already have a Parameter named '" + param.ProspectiveName + "'. Parameter names must be unique.");
                                }
                            }

                            if (isValidName) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds,
                                    "Undo change Parameter name.");
                                parameterRenaming.ParameterName = parameterRenaming.ProspectiveName;
                                parameterRenaming.IsEditing = false;
                                _isDirty = true;
                            }
                        }
                    }

                    if (parameterEditing != null) {
                        // ReSharper disable once ForCanBeConvertedToForeach
                        for (var c = 0; c < _sounds.realTimeParameters.Count; c++) {
                            var param = _sounds.realTimeParameters[c];
                            // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
                            if (parameterEditing == param) {
                                param.IsEditing = true;
                                param.ProspectiveName = param.ParameterName;
                            } else {
                                param.IsEditing = false;
                            }
                ...
                EditorGUILayout.BeginHorizontal();
                labelName = "Parameter Commands";
                if (!_sounds.showParameterCommands) {
                    labelName += " (" + _sounds.parameterCommands.Count + ")";
                }
                var newAmb = DTGUIHelper.Foldout(_sounds.showParameterCommands, labelName);
                if (newAmb != _sounds.showParameterCommands) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Parameter Commands");
                    _sounds.showParameterCommands = newAmb;
                }
                DTGUIHelper.AddHelpIconNoStyle("https://www.dtdevtools.com/docs/masteraudio/RealTimeParameterCommands.htm#ParameterCommands");
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.EndVertical();
                ...
                if (_sounds.showParameterCommands) {
                    ...
                    if (GUILayout.Button(new GUIContent("Add Parameter Command"), EditorStyles.toolbarButton, GUILayout.Width(160))) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "Add Parameter Command");
                        var initialName = ("NewParameterCommand " + (_sounds.parameterCommands.Count + 1));
                        MasterAudio.CreateRealTimeParameterCommand(initialName, MasterAudio.NoGroupName, true, 0f, 1000f, false, false);
                    }

                    if (_sounds.parameterCommands.Count > 0) {
                        var hasExpanded = false;
                        foreach (var t in _sounds.parameterCommands) {
                            if (!t.IsExpanded) {
                                continue;
                            }
                    ...
                    if (_sounds.parameterCommands.Count == 0) {
                        DTGUIHelper.ShowLargeBarAlert("You currently have no Parameter Commands set up.");
                    }

                    ParameterCommand parameterCommandEditing = null;
                    ParameterCommand parameterCommandRenaming = null;
                    int? paramCmdToRemove = null;

                    for (var i = 0; i < _sounds.parameterCommands.Count; i++) {
                        var aParamCmd = _sounds.parameterCommands[i];

                        DTGUIHelper.StartGroupHeader(1);

                        EditorGUILayout.BeginHorizontal();

                        if (!aParamCmd.IsEditing || Application.isPlaying) {
                            EditorGUI.indentLevel = 1;
                            labelName = aParamCmd.CommandName;
                            if (!aParamCmd.IsExpanded) {
                                labelName += " (" + aParamCmd.Elements.Count + " " + (aParamCmd.Elements.Count == 1 ? "Element" : "Elements") + ")";
                            }

                            var showStopButton = false;

                            if (Application.isPlaying) {
                                var activeInstances = MasterAudio.GetCommandTrackersOfCommandType(aParamCmd);
                                labelName += ", Active: " + activeInstances;
                                if (activeInstances > 0) {
                                    showStopButton = true;
                                }
                            }

                            var exp = DTGUIHelper.Foldout(aParamCmd.IsExpanded, labelName);
                            if (exp != aParamCmd.IsExpanded) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle expand Parameter Command");
                                aParamCmd.IsExpanded = exp;
                            }

                            if (showStopButton) {
                                if (GUILayout.Button(new GUIContent("Stop All"), EditorStyles.toolbarButton, GUILayout.Width(60))) {
                                    MasterAudio.StopParameterCommandsByCommandName(aParamCmd.CommandName);
                                }
                                GUILayout.Space(4);
                            }

                            if (Application.isPlaying) {
                                if (GUILayout.Button(new GUIContent("Invoke!"), EditorStyles.toolbarButton, GUILayout.Width(60))) {
                                    MasterAudio.InvokeParameterCommand(aParamCmd.CommandName, MasterAudio.SafeInstance.Trans);
                                }
                            }


                            if (!Application.isPlaying) {
                                var settingsIcon = new GUIContent(MasterAudioInspectorResources.GearTexture, "Click to edit Parameter Command Name");
                                if (GUILayout.Button(settingsIcon, EditorStyles.toolbarButton, GUILayout.Width(24), GUILayout.Height(16))) {
                                    parameterCommandEditing = aParamCmd;
                                }
                            }
                            ...
                            EditorGUI.indentLevel = 0;
                            GUI.backgroundColor = DTGUIHelper.BrightTextColor;
                            var tex = EditorGUILayout.TextField("", aParamCmd.ProspectiveName);
                            if (tex != aParamCmd.ProspectiveName) {
                                aParamCmd.ProspectiveName = tex;
                                _isDirty = true;
                            }

                            var buttonPressed = DTGUIHelper.AddCancelSaveButtons("Sound Event");

                            switch (buttonPressed) {
                                case DTGUIHelper.DTFunctionButtons.Cancel:
                                    aParamCmd.IsEditing = false;
                                    _isDirty = true;
                                    break;
                                case DTGUIHelper.DTFunctionButtons.Save:
                                    parameterCommandRenaming = aParamCmd;
                                    break;
                            }
                        }
                        EditorGUILayout.EndHorizontal();
                        ...
                        if (aParamCmd.IsExpanded) {
                            ...
                            var index = ParameterNameList.IndexOf(aParamCmd.ParameterName);
                            if (index == -1) {
                                index = 0;
                            }

                            var newIndex = EditorGUILayout.Popup("Parameter", index, ParameterNameList.ToArray());
                            if (newIndex >= 0) {
                                if (index != newIndex) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter");
                                }
                                aParamCmd.ParameterName = ParameterNameList[newIndex];
                            }

                            var isParamValid = aParamCmd.ParameterName != MasterAudio.NoGroupName;

                            if (!isParamValid) {
                                DTGUIHelper.ShowLargeBarAlert("Select a valid Parameter first. This Parameter Command will do nothing.");
                            } else {
                                GUI.contentColor = DTGUIHelper.BrightButtonColor;

                                RealTimeParameter selectedParam = null;
                                if (!MasterAudio.ParameterCommandHardCodedNames.Contains(aParamCmd.ParameterName)) {
                                    selectedParam = MasterAudio.GetRealTimeParameterFromCommand(aParamCmd);
                                }

                                var useStopRange = EditorGUILayout.Toggle(new GUIContent("Stop Cmd Out Of Range", "Check this box to make the Command auto-stop when the Parameter value falls outside the range specified below."), aParamCmd.StopCmdOutOfRange);
                                if (useStopRange != aParamCmd.StopCmdOutOfRange) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Stop Cmd Out Of Range");
                                    aParamCmd.StopCmdOutOfRange = useStopRange;
                                }

                                if (aParamCmd.StopCmdOutOfRange) {
                                    EditorGUI.indentLevel = 1;
                                    var newMinValidRange = EditorGUILayout.FloatField("Valid Range Min", aParamCmd.ValidRangeMin);
                                    if (newMinValidRange != aParamCmd.ValidRangeMin) {
                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Valid Range Min");
                                        aParamCmd.ValidRangeMin = newMinValidRange;
                                    }


                                    if (aParamCmd.ValidRangeMin > aParamCmd.ValidRangeMax) {
                                        aParamCmd.ValidRangeMax = aParamCmd.ValidRangeMin;
                                        _isDirty = true;
                                    }

                                    var newMaxValidRange = EditorGUILayout.FloatField("Valid Range Max", aParamCmd.ValidRangeMax);
                                    if (newMaxValidRange != aParamCmd.ValidRangeMax) {
                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Valid Range Max");
                                        aParamCmd.ValidRangeMax = newMaxValidRange;
                                    }

                                    if (aParamCmd.ValidRangeMax < aParamCmd.ValidRangeMin) {
                                        aParamCmd.ValidRangeMin = aParamCmd.ValidRangeMax;
                                        _isDirty = true;
                                    }
                                }
                                ...
                                if (selectedParam == null) {
                                    var newMinParamVal = EditorGUILayout.FloatField("Curve X Min", aParamCmd.MinDisplayCurveX);
                                    if (newMinParamVal != aParamCmd.MinDisplayCurveX) {
                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Curve X Min");
                                        aParamCmd.MinDisplayCurveX = newMinParamVal;
                                    }

                                    if (aParamCmd.MinDisplayCurveX > aParamCmd.MaxDisplayCurveX) {
                                        aParamCmd.MaxDisplayCurveX = aParamCmd.MinDisplayCurveX;
                                        _isDirty = true;
                                    }

                                    var newMaxParamVal = EditorGUILayout.FloatField("Curve X Max", aParamCmd.MaxDisplayCurveX);
                                    if (newMaxParamVal != aParamCmd.MaxDisplayCurveX) {
                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Curve X Max");
                                        aParamCmd.MaxDisplayCurveX = newMaxParamVal;
                                    }

                                    if (aParamCmd.MaxDisplayCurveX < aParamCmd.MinDisplayCurveX) {
                                        aParamCmd.MinDisplayCurveX = aParamCmd.MaxDisplayCurveX;
                                        _isDirty = true;
                                    }
                                }
                                ...
                                var curveMaxX = 1000f;
                                if (selectedParam != null) {
                                    curveMinX = selectedParam.MinValue;
                                    curveMaxX = Math.Abs(selectedParam.MaxValue - selectedParam.MinValue);
                                } else {
                                    curveMinX = aParamCmd.MinDisplayCurveX;
                                    curveMaxX = aParamCmd.MaxDisplayCurveX;
                                }

                                EditorGUILayout.BeginHorizontal();
                                GUILayout.Space(2);

                                if (GUILayout.Button(new GUIContent("Add Element"), EditorStyles.toolbarButton, GUILayout.Width(110))) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "Add Element");
                                    MasterAudio.CreateParameterCommandElement(aParamCmd, 
                                        "YourElement" + (aParamCmd.Elements.Count + 1),
                                        "YourVariation",
                                        true,
                                        false,
                                        EventSounds.VariationType.PlayRandom,
                                        MasterAudio.NoGroupName,
                                        1f, false, 1f, 0f, 0f, 1f, 9f, 10f,
                                        MasterAudio.SoundSpawnLocationMode.AttachToCaller,
                                        true,
                                        false,
                                        new AnimationCurve(),
                                        false,
                                        new AnimationCurve(),
                                        false, false, new AnimationCurve(), false, new AnimationCurve(), 
                                        1f, false);
                                }

                                var hasExpanded2 = false;
                                foreach (var t in aParamCmd.Elements) {
                                    if (!t.IsExpanded) {
                                        continue;
                                    }
                                ...
                                var theButtonText2 = hasExpanded2 ? "Collapse All" : "Expand All";

                                GUILayout.Space(6);

                                if (GUILayout.Button(new GUIContent(theButtonText2), EditorStyles.toolbarButton, GUILayout.Width(100))) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "Expand / Collapse All");
                                    var newExpandState = hasExpanded2 ? false : true;
                                ...
                                if (aParamCmd.Elements.Count == 0) {
                                    DTGUIHelper.ShowLargeBarAlert("You currently have no Elements in this Parameter Command. It will do nothing.");
                                }

                                ParameterCommandElement elementEditing = null;
                                ParameterCommandElement elementRenaming = null;
                                int? elementToRemove = null;

                                for (var d = 0; d < aParamCmd.Elements.Count; d++) {
                                    var element = aParamCmd.Elements[d];

                                    DTGUIHelper.StartGroupHeader(2);

                                    EditorGUILayout.BeginHorizontal();

                                    if (!element.IsEditing || Application.isPlaying) {
                                        EditorGUI.indentLevel = 1;

                                        var elementLabel = element.ElementName;

                                        if (!element.IsExpanded && element.SoundGroup != MasterAudio.NoGroupName) {
                                            elementLabel += " (" + element.SoundGroup + ")";
                                        }

                                        var exp = DTGUIHelper.Foldout(element.IsExpanded, elementLabel);
                                        if (exp != element.IsExpanded) {
                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle expand Element");
                                            element.IsExpanded = exp;
                                        }

                                        if (!Application.isPlaying) {
                                            var settingsIcon = new GUIContent(MasterAudioInspectorResources.GearTexture, "Click to edit Element Name");
                                            if (GUILayout.Button(settingsIcon, EditorStyles.toolbarButton, GUILayout.Width(24), GUILayout.Height(16))) {
                                        ...
                                        if (!Application.isPlaying) {
                                            if (DTGUIHelper.AddDeleteIcon("Element")) {
                                                elementToRemove = d;
                                            }
                                        }
                                    } else {
                                        EditorGUI.indentLevel = 0;
                                        GUI.backgroundColor = DTGUIHelper.BrightTextColor;
                                        var tex = EditorGUILayout.TextField("", element.ProspectiveName);
                                        if (tex != element.ProspectiveName) {
                                            element.ProspectiveName = tex;
                                            _isDirty = true;
                                        }

                                        var buttonPressed = DTGUIHelper.AddCancelSaveButtons("Element");
                                    ...
                                    if (element.IsExpanded) {
                                        ...
                                        var isValidElementSound = element.SoundGroup != MasterAudio.NoGroupName;

                                        if (isValidElementSound) {
                                            DTGUIHelper.StartGroupHeader(2, false);
                                            var curveLabel = "Show Animation Curves";
                                            if (!element.ShowCurves) {
                                                var curvesUsed = 0;
                                                if (element.AffectHighPassFilter) {
                                                    curvesUsed++;
                                                }
                                                if (element.AffectLowPassFilter) {
                                                    curvesUsed++;
                                                }
                                                if (element.AffectPitch) {
                                                    curvesUsed++;
                                                }
                                                if (element.AffectVolume) {
                                                    curvesUsed++;
                                                }

                                                curveLabel += " (" + curvesUsed + ")";
                                            }

                                            var newShowCurves = EditorGUILayout.Toggle(new GUIContent(curveLabel),  element.ShowCurves);
                                            if (newShowCurves != element.ShowCurves) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Show Animation Curves");
                                                element.ShowCurves = newShowCurves;
                                            }

                                            if (!element.AffectPitch && !element.AffectVolume && !element.AffectHighPassFilter && !element.AffectLowPassFilter) {
                                                DTGUIHelper.ShowLargeBarAlert("No Animation Curves are used. Parameter changes will not affect this Element.");
                                            }

                                            if (element.ShowCurves) {
                                                DTGUIHelper.StartGroupHeader(2, false);
                                                var newAffectVolume = EditorGUILayout.Toggle(new GUIContent("Affect Volume", "Check this box to have Parameter changes affect the volume in real time."),
                                                    element.AffectVolume);
                                                if (newAffectVolume != element.AffectVolume) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Affect Volume");
                                                    element.AffectVolume = newAffectVolume;
                                                }

                                                if (element.AffectVolume) {
                                                    DTGUIHelper.BeginGroupedControls();
                                                    var newVolCurve = EditorGUILayout.CurveField(
                                                        new GUIContent("Volume Curve", "This curve controls the volume of selected Sound Group based on the changing value of selected Parameter."),
                                                        element.VolumeAnimationCurve, Color.green,
                                                        new Rect(curveMinX, 0, curveMaxX, 1));
                                                    if (newVolCurve != element.VolumeAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Volume Curve");
                                                        element.VolumeAnimationCurve = newVolCurve;
                                                    }

                                                    var newShowRange = EditorGUILayout.Toggle("Fade In/Out Fields", element.ShowFadeInOutFields);
                                                    if (newShowRange != element.ShowFadeInOutFields) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Fade In/Out Fields");
                                                        element.ShowFadeInOutFields = newShowRange;
                                                    }

                                                    if (element.ShowFadeInOutFields) {
                                                        var label = "Fade In Param Value Start";
                                                        var newFadeInStart = EditorGUILayout.FloatField(label, element.FadeInRangeStart);
                                                        if (newFadeInStart != element.FadeInRangeStart) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change " + label);
                                                            element.FadeInRangeStart = newFadeInStart;
                                                        }
                                                        if (selectedParam != null && element.FadeInRangeStart < selectedParam.MinValue) {
                                                            DTGUIHelper.ShowColorWarning("Fade In Range Start is less than selected Parameter's Min.");
                                                        }

                                                        if (element.FadeInRangeStart > element.FadeInRangeEnd) {
                                                            element.FadeInRangeEnd = element.FadeInRangeStart;
                                                            _isDirty = true;
                                                        }

                                                        label = "Fade In Param Value End";
                                                        var newFadeInEnd = EditorGUILayout.FloatField(label, element.FadeInRangeEnd);
                                                        if (newFadeInEnd != element.FadeInRangeEnd) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change " + label);
                                                            element.FadeInRangeEnd = newFadeInEnd;
                                                        }

                                                        if (element.FadeInRangeEnd < element.FadeInRangeStart) {
                                                            element.FadeInRangeStart = element.FadeInRangeEnd;
                                                            _isDirty = true;
                                                        }

                                                        label = "Fade Out Param Value Start";
                                                        var newFadeOutStart = EditorGUILayout.FloatField(label, element.FadeOutRangeStart);
                                                        if (newFadeOutStart != element.FadeOutRangeStart) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change " + label);
                                                            element.FadeOutRangeStart = newFadeOutStart;
                                                        }

                                                        if (element.FadeOutRangeStart > element.FadeOutRangeEnd) {
                                                            element.FadeOutRangeEnd = element.FadeOutRangeStart;
                                                            _isDirty = true;
                                                        }

                                                        if (element.FadeOutRangeStart < element.FadeInRangeEnd) {
                                                            element.FadeOutRangeStart = element.FadeInRangeEnd;
                                                            _isDirty = true;
                                                        }

                                                        if (element.FadeOutRangeEnd < element.FadeInRangeEnd) {
                                                            element.FadeOutRangeEnd = element.FadeInRangeEnd;
                                                            _isDirty = true;
                                                        }

                                                        label = "Fade Out Param Value End";
                                                        var newFadeOutEnd = EditorGUILayout.FloatField(label, element.FadeOutRangeEnd);
                                                        if (newFadeOutEnd != element.FadeOutRangeEnd) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change " + label);
                                                            element.FadeOutRangeEnd = newFadeOutEnd;
                                                        }

                                                        if (selectedParam != null && element.FadeOutRangeEnd > selectedParam.MaxValue) {
                                                            DTGUIHelper.ShowColorWarning("Fade Out Range End is greater than selected Parameter's Max.");
                                                        }

                                                        if (element.FadeOutRangeEnd < element.FadeOutRangeStart) {
                                                            element.FadeOutRangeStart = element.FadeOutRangeEnd;
                                                            _isDirty = true;
                                                        }

                                                        label = "Max Volume";
                                                        var newMaxVol = EditorGUILayout.Slider(label, element.MaxDisplayVolumeCurveY, 0f, 1f);
                                                        if (newMaxVol != element.MaxDisplayVolumeCurveY) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change " + label);
                                                            element.MaxDisplayVolumeCurveY = newMaxVol;
                                                        }

                                                        if (GUILayout.Button(new GUIContent("Set Curve To Fade In/Out Fields", "This will set the curve to your Fade In/Out fields, with max volume set to the Volume field below in Advanced Controls."), EditorStyles.toolbarButton, GUILayout.Width(190))) {
                                                            while (element.VolumeAnimationCurve.keys.Count() > 0) {
                                                                element.VolumeAnimationCurve.RemoveKey(0);
                                                            }
                                                            element.VolumeAnimationCurve.AddKey(element.FadeInRangeStart, 0);
                                                            element.VolumeAnimationCurve.AddKey(element.FadeInRangeEnd, element.MaxDisplayVolumeCurveY);
                                                            element.VolumeAnimationCurve.AddKey(element.FadeOutRangeStart, element.MaxDisplayVolumeCurveY);
                                                            element.VolumeAnimationCurve.AddKey(element.FadeOutRangeEnd, 0);
                                                            AnimationUtility.SetKeyRightTangentMode(element.VolumeAnimationCurve, 1, AnimationUtility.TangentMode.Linear);
                                                            AnimationUtility.SetKeyLeftTangentMode(element.VolumeAnimationCurve, 2, AnimationUtility.TangentMode.Linear);
                                                        }
                                                    }
                                                    DTGUIHelper.EndGroupedControls();
                                                }
                                                ...
                                                DTGUIHelper.StartGroupHeader(2, false);
                                                var newAffectPitch = EditorGUILayout.Toggle(new GUIContent("Affect Pitch", "Check this box to have Parameter changes affect the pitch in real time."),
                                                    element.AffectPitch);
                                                if (newAffectPitch != element.AffectPitch) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Affect Pitch");
                                                    element.AffectPitch = newAffectPitch;
                                                }

                                                if (element.AffectPitch) {
                                                    DTGUIHelper.BeginGroupedControls();
                                                    var newPitchCurve = EditorGUILayout.CurveField(new GUIContent("Pitch Curve", "This curve controls the pitch of selected Sound Group based on the changing value of selected Parameter."),
                                                        element.PitchAnimationCurve, Color.yellow,
                                                        new Rect(curveMinX, 0, curveMaxX, 3));
                                                    if (newPitchCurve != element.PitchAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Pitch Curve");
                                                        element.PitchAnimationCurve = newPitchCurve;
                                                    }
                                                    DTGUIHelper.EndGroupedControls();
                                                }
                                                EditorGUILayout.EndVertical();
                                                ...
                                                DTGUIHelper.StartGroupHeader(2, false);
                                                var newAffectLowPass = EditorGUILayout.Toggle(new GUIContent("Affect Low Pass Filter", "Check this box to have Parameter changes affect the Low Pass Filter cutoff frequency in real time."),
                                                    element.AffectLowPassFilter);
                                                if (newAffectLowPass != element.AffectLowPassFilter) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Affect Low Pass Filter");
                                                    element.AffectLowPassFilter = newAffectLowPass;
                                                }

                                                if (element.AffectLowPassFilter) {
                                                    DTGUIHelper.BeginGroupedControls();
                                                    var newLowPassCurve = EditorGUILayout.CurveField(new GUIContent("Low Pass Filter Curve", "This curve controls the Low Pass Filter cutoff frequency of selected Sound Group based on the changing value of selected Parameter."),
                                                        element.LowPassAnimationCurve, Color.yellow,
                                                        new Rect(curveMinX, 10, curveMaxX, 22000));
                                                    if (newLowPassCurve != element.LowPassAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Low Pass Filter Curve");
                                                        element.LowPassAnimationCurve = newLowPassCurve;
                                                    }
                                                    DTGUIHelper.EndGroupedControls();
                                                }
                                                EditorGUILayout.EndVertical();
                                                ...
                                                DTGUIHelper.StartGroupHeader(2, false);
                                                var newAffectHighPass = EditorGUILayout.Toggle(new GUIContent("Affect High Pass Filter", "Check this box to have Parameter changes affect the High Pass Filter cutoff frequency in real time."),
                                                    element.AffectHighPassFilter);
                                                if (newAffectHighPass != element.AffectHighPassFilter) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Affect High Pass Filter");
                                                    element.AffectHighPassFilter = newAffectHighPass;
                                                }

                                                if (element.AffectHighPassFilter) {
                                                    DTGUIHelper.BeginGroupedControls();
                                                    var newHighPassCurve = EditorGUILayout.CurveField(new GUIContent("High Pass Filter Curve", "This curve controls the High Pass Filter cutoff frequency of selected Sound Group based on the changing value of selected Parameter."),
                                                        element.HighPassAnimationCurve, Color.yellow,
                                                        new Rect(curveMinX, 10, curveMaxX, 22000));
                                                    if (newHighPassCurve != element.HighPassAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change High Pass Filter Curve");
                                                        element.HighPassAnimationCurve = newHighPassCurve;
                                                    }
                                                    DTGUIHelper.EndGroupedControls();
                                                }
                                                EditorGUILayout.EndVertical();
                                        ...
                                        var elementSoundGroupIndex = groupNameList.IndexOf(element.SoundGroup);
                                        var elementNewSoundGroupIndex = EditorGUILayout.Popup("Sound Group", elementSoundGroupIndex, groupNameList.ToArray());
                                        if (elementNewSoundGroupIndex < 0) {
                                            elementNewSoundGroupIndex = 0;
                                            _isDirty = true;
                                        }

                                        if (elementNewSoundGroupIndex >= 0) {
                                            if (elementSoundGroupIndex != elementNewSoundGroupIndex) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Element Sound Group");
                                                CalculateRadiusIfSelected(element);
                                            }
                                            element.SoundGroup = groupNameList[elementNewSoundGroupIndex];
                                        }

                                        if (isValidElementSound) {
                                            var button = DTGUIHelper.AddSettingsButton("Element Sound Group");
                                            switch (button) {
                                                case DTGUIHelper.DTFunctionButtons.Go:
                                                    var grp = groupNameList[elementNewSoundGroupIndex];
                                            ...
                                            var sType = GroupNameList[elementNewSoundGroupIndex];
                                        ...
                                        if (!isValidElementSound) {
                                            DTGUIHelper.ShowLargeBarAlert("Select a valid Sound Group. This Element will do nothing.");
                                        } else {
                                            var newShowAdv = EditorGUILayout.Toggle("Advanced Controls", element.ShowAdvanced);
                                            if (newShowAdv != element.ShowAdvanced) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Advanced Controls");
                                                element.ShowAdvanced = newShowAdv;
                                            }

                                            if (element.ShowAdvanced) {
                                                var newVarType = (EventSounds.VariationType)EditorGUILayout.EnumPopup("Variation Mode", element.VariationMode);
                                                if (newVarType != element.VariationMode) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Variation Mode");
                                                    element.VariationMode = newVarType;
                                                }

                                                if (element.VariationMode == EventSounds.VariationType.PlaySpecific) {
                                                    var newVariation = EditorGUILayout.TextField("Variation Name", element.VariationName);
                                                    if (newVariation != element.VariationName) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Variation Name");
                                                        element.VariationName = newVariation;
                                                        CalculateRadiusIfSelected(element);
                                                    }

                                                    if (string.IsNullOrWhiteSpace(element.VariationName)) {
                                                        DTGUIHelper.ShowLargeBarAlert("Variation Name cannot be blank. This Element will do nothing.");
                                                    }
                                                }

                                                var aud = _sounds.GetNamedOrFirstAudioSource(element);
                                                if (aud == null) {
                                                    if (element.VariationMode == EventSounds.VariationType.PlaySpecific) {
                                                        DTGUIHelper.ShowLargeBarAlert("Variation Name could not be found. This Element will do nothing.");
                                                        _sounds.parameterCommandToGizmo = null;
                                                        element.ShowSphereGizmo = false;
                                                    }
                                                }

                                                var newSpawnMode = (MasterAudio.SoundSpawnLocationMode)EditorGUILayout.EnumPopup("Sound Spawn Mode", element.SoundSpawnLocationMode);
                                                if (newSpawnMode != element.SoundSpawnLocationMode) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Sound Spawn Mode");
                                                    element.SoundSpawnLocationMode = newSpawnMode;
                                                }

                                                if (!element.AffectVolume) {
                                                    var newVol = DTGUIHelper.DisplayVolumeField(element.Volume,
                                                        DTGUIHelper.VolumeFieldType.None, MasterAudio.MixerWidthMode.Normal, 0f, true);
                                                    if (newVol != element.Volume) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Volume");
                                                ...
                                                if (!element.AffectPitch) {
                                                    var newUseSpecificPitch = EditorGUILayout.Toggle(new GUIContent("Override Pitch", "Turn this on only to override the pitch of the Variation"), element.OverridePitch);
                                                    if (newUseSpecificPitch != element.OverridePitch) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Override Pitch");
                                                        element.OverridePitch = newUseSpecificPitch;
                                                    }

                                                    if (element.OverridePitch) {
                                                        var newPitch = DTGUIHelper.DisplayPitchField(element.Pitch);
                                                        if (newPitch != element.Pitch) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Pitch");
                                                            element.Pitch = newPitch;
                                                        }
                                                    }
                                                }

                                                if (aud != null) {
                                                    var newShowgiz = EditorGUILayout.Toggle("Adjust Audio Range", element.ShowSphereGizmo);
                                                    if (newShowgiz != element.ShowSphereGizmo) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Adjust Audio Range");
                                                        if (newShowgiz == true) {
                                                            // turn rest off
                                                            TurnOffAllOtherShowRangeGizmo();

                                                            _sounds.showParameterCommandGizmo = true;
                                                            _sounds.parameterCommandToGizmo = element;
                                                        } else if (_sounds.parameterCommandToGizmo == element) {
                                                            _sounds.showParameterCommandGizmo = false;
                                                            _sounds.parameterCommandToGizmo = null;
                                                        }

                                                        element.ShowSphereGizmo = newShowgiz;
                                                        CalculateRadiusIfSelected(element);
                                                    }

                                                    if (element.ShowSphereGizmo) {
                                                        ...
                                                        if (newMin != aud.minDistance) {
                                                            ...
                                                            switch (element.VariationMode) {
                                                                case EventSounds.VariationType.PlayRandom:
                                                                    var sources = _sounds.GetAllVariationAudioSources(element);
                                                                    if (sources != null) {
                                                        ...
                                                        switch (element.VariationMode) {
                                                            case EventSounds.VariationType.PlayRandom:
                                                                DTGUIHelper.ShowLargeBarAlert("Adjusting the Min/Max Distance fields will change the Min/Max Distance on the Audio Source of every Variation in the selected Sound Group.");
                                                                break;
                                                            case EventSounds.VariationType.PlaySpecific:
                                                                DTGUIHelper.ShowLargeBarAlert("Adjusting the Min/Max Distance fields will change the Min/Max Distance on the Audio Source for the selected Variation in the selected Sound Group.");
                                                                break;
                                                        }
                                                ...
                                                var newDelay = EditorGUILayout.Slider("Delay Sound (sec)", element.DelaySound, 0f, 10f);
                                                if (newDelay != element.DelaySound) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Delay Sound");
                                                    element.DelaySound = newDelay;
                                                }
                                            }
                                        }
                                        EditorGUILayout.EndVertical();
                                ...
                                if (elementToRemove.HasValue) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "delete Element");
                                    aParamCmd.Elements.RemoveAt(elementToRemove.Value);
                                }

                                if (elementEditing != null) {
                                    // ReSharper disable once ForCanBeConvertedToForeach
                                    for (var c = 0; c < aParamCmd.Elements.Count; c++) {
                                        var element = aParamCmd.Elements[c];
                                        // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
                                        if (elementEditing == element) {
                                            element.IsEditing = true;
                                            element.ProspectiveName = element.ElementName;
                                        } else {
                                            element.IsEditing = false;
                                        }
                                ...
                                if (elementRenaming != null) {
                                    var isValidName = true;

                                    if (string.IsNullOrEmpty(elementRenaming.ProspectiveName)) {
                                        isValidName = false;
                                        DTGUIHelper.ShowAlert("You cannot have a blank Element name.");
                                    }
                                    ...
                                    if (isValidName) {
                                        ...
                                        for (var c = 0; c < aParamCmd.Elements.Count; c++) {
                                            var element = aParamCmd.Elements[c];
                                            // ReSharper disable once InvertIf
                                            if (element != elementRenaming && element.ElementName == elementRenaming.ProspectiveName) {
                                                isValidName = false;
                                                DTGUIHelper.ShowAlert("You already have an Element named '" + aParamCmd.ProspectiveName + "'. Element names must be unique per Sound Event.");
                                            }
                                        }

                                        if (isValidName) {
                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds,
                                                "Undo change Element name.");
                                            elementRenaming.ElementName = elementRenaming.ProspectiveName;
                                            elementRenaming.IsEditing = false;
                                            _isDirty = true;
                                        }
                                    }
                                }
                    ...
                    if (paramCmdToRemove.HasValue) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "delete Parameter Command");
                        _sounds.parameterCommands.RemoveAt(paramCmdToRemove.Value);
                    }

                    if (parameterCommandEditing != null) {
                        // ReSharper disable once ForCanBeConvertedToForeach
                        for (var c = 0; c < _sounds.parameterCommands.Count; c++) {
                            var anEvent = _sounds.parameterCommands[c];
                            // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
                            if (parameterCommandEditing == anEvent) {
                                anEvent.IsEditing = true;
                                anEvent.ProspectiveName = anEvent.CommandName;
                            } else {
                                anEvent.IsEditing = false;
                            }
                    ...
                    if (parameterCommandRenaming != null) {
                        var isValidName = true;

                        if (string.IsNullOrEmpty(parameterCommandRenaming.ProspectiveName)) {
                            isValidName = false;
                            DTGUIHelper.ShowAlert("You cannot have a blank Parameter Command name.");
                        }
                        ...
                        if (isValidName) {
                            ...
                            for (var c = 0; c < _sounds.parameterCommands.Count; c++) {
                                var anEvent = _sounds.parameterCommands[c];
                                // ReSharper disable once InvertIf
                                if (anEvent != parameterCommandRenaming && anEvent.CommandName == parameterCommandRenaming.ProspectiveName) {
                                    isValidName = false;
                                    DTGUIHelper.ShowAlert("You already have a Parameter Command named '" + anEvent.ProspectiveName + "'. Parameter Command names must be unique.");
                                }
                            }

                            if (isValidName) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds,
                                    "Undo change Parameter Command name.");
                                parameterCommandRenaming.CommandName = parameterCommandRenaming.ProspectiveName;
                                parameterCommandRenaming.IsEditing = false;
                                _isDirty = true;
                            }
                        }
                    }
...
#endif
        private void CalculateRadiusIfSelected(ParameterCommandElement element) {
            if (element.ShowSphereGizmo) {
                _sounds.CalculateRadius(element);
            }
        }
        ...
        private static MasterAudioGroup CreateSoundGroupFromTemplate(AudioClip aClip, int groupTemplateIndex) {
            ...
            for (var i = 0; i < newGroup.transform.childCount; i++) {
                ...
                } else if (_sounds.addSteamAudioSources && SteamAudioHelper.DarkTonicSteamAudioPackageInstalled()) {
                    SteamAudioHelper.AddSteamAudioSourceToVariation(variation);
        ...
        private List<string> ParameterNameList {
            get {
                ...
                parameters.AddRange(MasterAudio.ParameterHardCodedNames);
                parameters.RemoveAt(0);

                foreach (var p in _sounds.realTimeParameters) {
        ...
        public static void DisplayJukebox(List<PlaylistController> controllers, List<string> playlistNames) {
            ...
            foreach (var t in controllers) {
                ...
                GUILayout.Space(3);
                buttonPressed = DTGUIHelper.AddJukeboxIcons(pl);
                if (playingSource != null) {
        ...
        private void TurnOffAllOtherShowRangeGizmo() {
            foreach (var paramCmd in _sounds.parameterCommands) {
                foreach (var element in paramCmd.Elements) {
                    element.ShowSphereGizmo = false;
                }
            }
        }
