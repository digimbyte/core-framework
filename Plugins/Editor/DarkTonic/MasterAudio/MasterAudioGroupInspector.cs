        ...
        if (newDelay != _group.addressableUnusedSecondsLifespan) {
            ...
            if (!Application.isPlaying)
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("Actions", EditorStyles.wordWrappedLabel, GUILayout.Width(55f));
                    GUILayout.Space(30);
                    GUI.contentColor = DTGUIHelper.BrightButtonColor;
                ...
                for (var i = 0; i < _group.groupVariations.Count; i++)
                {
                    ...
                    if (!isVideoPlayersGroup)
                    {
                        ...
                        if (variation.VarAudio.loop) {
                            var newReRandomize = EditorGUILayout.Toggle("Re-randomize On Loop", variation.reReRandomizeOnLoop);
                            if (newReRandomize != variation.reReRandomizeOnLoop) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref varIsDirty, variation, "toggle Re-randomize On Loop");
                                variation.reReRandomizeOnLoop = newReRandomize;
                            }
                        }
