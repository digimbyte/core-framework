    ...
    public class AudioEventInspector : Editor
    {
            ...
            List<string> parameterNames)
        {
            ...
            if (parameterCommands == null) {
                parameterCommands = new List<string>();
            }
            ...
            foreach (var dsgc in creators)
            {
                ...
                foreach (var paramCmd in dsgc.parameterCommands) {
                    parameterCommands.Add(paramCmd.CommandName);
                }

                foreach (var parameter in  dsgc.realTimeParameters) {
            ...
            parameterCommands.Sort();
            if (parameterCommands.Count > 1) {
                parameterCommands.Insert(0, parameterCommands[1]);
                parameterCommands.RemoveAt(2);
            }

            parameterNames.Sort();
            if (parameterNames.Count > 1) {
                parameterNames.Insert(0, parameterNames[2]);
                parameterNames.RemoveAt(3);
                parameterNames.Insert(0, parameterNames[4]);
                parameterNames.RemoveAt(5);
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            PopulateItemNames(_groupNames, _busNames, _playlistNames, _customEventNames, _parameterCmdNames, _parameterNames);
            ...
            if (!_sounds.useDestroySound) {
                unusedEventTypes.Add("Destroy");
            }
...
#pragma warning restore 162
                ...
                if (newEventIndex > -1)
                {
                    ...
                    switch (selectedEvent)
                    {
                        ...
                        case "Disable":
                            ...
                            break;
                        case "Destroy":
                            _sounds.useDestroySound = true;
                            AddEventIfZero(_sounds.destroySound);
                            break;
                        case "Visible":
            ...
            if (GUILayout.Button("Collapse All Events", EditorStyles.toolbarButton, GUILayout.Width(140)))
            {
                ...
                _sounds.destroySound.isExpanded = false;
            ...
            if (_sounds.useDestroySound) {
                RenderEventWithHeader("Destroy" + DisabledText, "toggle Destroy Sound", _sounds.destroySound, EventSounds.EventType.OnDestroy);
            }
                    ...
                    if (DTGUIHelper.AddDeleteIcon(false, "Trigger") == DTGUIHelper.DTFunctionButtons.Remove)
                    {
                        ...
                        switch (eType)
                        {
                            ...
                            case EventSounds.EventType.OnEnable:
                                ...
                                break;
                            case EventSounds.EventType.OnDestroy:
                                _sounds.useDestroySound = false;
                                break;
        ...
        private void FireEvent(EventSounds.EventType eType)
        {
            switch (eType)
            {
                ...
                case EventSounds.EventType.OnEnable:
                    ...
                    break;
                case EventSounds.EventType.OnDestroy:
                    _sounds.PlaySounds(_sounds.destroySound, EventSounds.EventType.OnDestroy);
                    break;
                case EventSounds.EventType.OnInvisible:
    ...
    "Turn this off to disable notification if a Custom Event fires more than once per frame. Only the first firing will do anything regardless."), aEvent.logDupeEventFiring);
                            ...
                            case MasterAudio.EventSoundFunctionType.ParameterCommandControl:
                                ...
                                var newParamCmdCommand = (MasterAudio.ParameterCmdCommand)EditorGUILayout.EnumPopup("Parameter Cmd Command", aEvent.currentParameterCmdCommand);
                                if (newParamCmdCommand != aEvent.currentParameterCmdCommand) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Cmd Command");
                                    aEvent.currentParameterCmdCommand = newParamCmdCommand;
                                }
                                ...
                                switch (aEvent.currentParameterCmdCommand) {
                                    ...
                                    default:
                                        if (_maInScene) {
                                            ...
                                            if (existingIndex >= 1) {
                                                EditorGUILayout.BeginHorizontal();
                                                paramCmdIndex = EditorGUILayout.Popup("Parameter Command", existingIndex, _parameterCmdNames.ToArray());
                                                if (existingIndex == 1) {
                                                    noCmd = true;
                                                }

                                                EditorGUILayout.EndHorizontal();
                                            } else if (existingIndex == -1 && aEvent.parameterCommandName == MasterAudio.NoGroupName) {
                                                paramCmdIndex = EditorGUILayout.Popup("Parameter Command", existingIndex, _parameterCmdNames.ToArray());
                                            } else { // non-match
                                                noMatch = true;
                                                var newCommand = EditorGUILayout.TextField("Parameter Command", aEvent.parameterCommandName);
                                                if (newCommand != aEvent.parameterCommandName) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Command");
                                                    aEvent.parameterCommandName = newCommand;
                                                }

                                                var newIndex = EditorGUILayout.Popup("All Parameter Commands", -1, _parameterCmdNames.ToArray());
                                                if (newIndex >= 0) {
                                            ...
                                            if (noCmd) {
                                                DTGUIHelper.ShowRedError("No Parameter Command specified. Action will do nothing.");
                                            } else if (noMatch) {
                                                DTGUIHelper.ShowRedError("Parameter Command found no match. Type in or choose one.");
                                            }
                                            ...
                                            if (paramCmdIndex.HasValue) {
                                                ...
                                                switch (paramCmdIndex.Value) {
                                                    case -1:
                                                        ...
                                                        break;
                                                    default:
                                                        aEvent.parameterCommandName = _parameterCmdNames[paramCmdIndex.Value];
                                                        break;
                                                }
                                            }
                                        } else {
                                            var newParamCmd = EditorGUILayout.TextField("Parameter Command", aEvent.parameterCommandName);
                                            if (newParamCmd != aEvent.parameterCommandName) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Command");
                                                aEvent.parameterCommandName = newParamCmd;
                                            }
                                        }
                            ...
                            case MasterAudio.EventSoundFunctionType.ParameterControl:
                                ...
                                var newParamCommand = (MasterAudio.RealTimeParameterCommand)EditorGUILayout.EnumPopup("Parameter Command", aEvent.currentParameterCommand);
                                if (newParamCommand != aEvent.currentParameterCommand) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Command");
                                    aEvent.currentParameterCommand = newParamCommand;
                                }

                                switch (aEvent.currentParameterCommand) {
                                    case MasterAudio.RealTimeParameterCommand.None:
                                        DTGUIHelper.ShowRedError("You have no Parameter selected. Action will do nothing.");
                                        break;
                                    default:
                                        if (_maInScene) {
                                            var existingIndex = _parameterNames.IndexOf(aEvent.parameterName);
                                            ...
                                            if (existingIndex >= 1) {
                                                EditorGUILayout.BeginHorizontal();
                                                paramIndex = EditorGUILayout.Popup("Parameter", existingIndex, _parameterNames.ToArray());
                                                if (existingIndex == 1) {
                                                    noParam = true;
                                                }

                                                EditorGUILayout.EndHorizontal();
                                            } else if (existingIndex == -1 && aEvent.parameterName == MasterAudio.NoGroupName) {
                                                paramIndex = EditorGUILayout.Popup("Parameter", existingIndex, _parameterNames.ToArray());
                                            } else { // non-match
                                                noMatch = true;
                                                var newParam = EditorGUILayout.TextField("Parameter", aEvent.parameterName);
                                                if (newParam != aEvent.parameterName) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter");
                                                    aEvent.parameterName = newParam;
                                                }

                                                var newIndex = EditorGUILayout.Popup("All Parameters", -1, _parameterNames.ToArray());
                                                if (newIndex >= 0) {
                                                    paramIndex = newIndex;
                                                }
                                            }
                                            ...
                                            if (noParam) {
                                                DTGUIHelper.ShowRedError("No Parameter specified. Action will do nothing.");
                                            } else if (noMatch) {
                                                DTGUIHelper.ShowRedError("Parameter found no match. Type in or choose one.");
                                            }

                                            if (paramIndex.HasValue) {
                                                if (existingIndex != paramIndex.Value) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter");
                                                }
                                                switch (paramIndex.Value) {
                                                    case -1:
                                                        aEvent.parameterName = MasterAudio.NoGroupName;
                                                        break;
                                                    default:
                                                        aEvent.parameterName = _parameterNames[paramIndex.Value];
                                                        break;
                                                }
                                            }
                                        } else {
                                        ...
                                        switch (aEvent.parameterName) {
                                            ...
                                            case MasterAudio.MagicParameterSecondsSinceStart:
                                                DTGUIHelper.ShowRedError("Built in Parameters cannot be modified. Select a different one.");
                                                break;
                                            default:
                                                var chosenParam = MasterAudio.GetRealTimeParameterByName(aEvent.parameterName);

                                                if (chosenParam == null) {
                                                    var newParamVal = EditorGUILayout.FloatField("Parameter Value", aEvent.parameterNewValue);
                                                    if (newParamVal != aEvent.parameterNewValue) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Value");
                                                        aEvent.parameterNewValue = newParamVal;
                                                    }
                                                } else {
                                                    var newParamVal = EditorGUILayout.Slider("Parameter Value", aEvent.parameterNewValue, chosenParam.MinValue, chosenParam.MaxValue);
                                                    if (newParamVal != aEvent.parameterNewValue) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Value");
                                                        aEvent.parameterNewValue = newParamVal;
                                                    }
        ...
        private void TurnOffAllOtherShowRangeGizmo()
        {
            ...
            TurnOfAllShowGizmoInEventGroup(_sounds.destroySound);
