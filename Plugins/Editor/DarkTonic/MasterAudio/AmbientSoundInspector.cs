...
namespace DarkTonic.MasterAudio.EditorScripts
{
    ...
    public class AmbientSoundInspector : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            var newShowgiz = EditorGUILayout.Toggle("Adjust Audio Range", _sounds.showSphereGizmo);
            if (newShowgiz != _sounds.showSphereGizmo) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "toggle Adjust Audio Range");
                _sounds.showSphereGizmo = newShowgiz;

                if (newShowgiz) {
                    _sounds.ambientToGizmo = _sounds;
                } else {
                    _sounds.ambientToGizmo = null;
                }

                UnityEditorInternal.InternalEditorUtility.RepaintAllViews();
            }

            if (_sounds.showSphereGizmo) {
