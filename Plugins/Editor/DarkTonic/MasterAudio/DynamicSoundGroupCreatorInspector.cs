...
namespace DarkTonic.MasterAudio.EditorScripts
{
    ...
    public class DynamicSoundGroupCreatorInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var state = _creator.soundGroupsAreExpanded;
            var text = "Dynamic Group Mixer";
            ...
            if (_creator.soundGroupsAreExpanded)
            {
                ...
                for (var i = 0; i < _groups.Count; i++)
                {
                    ...
                    GUILayout.Label(aGroup.name, GUILayout.MinWidth(40));
                    ...
                    for (var i = 0; i < _creator.groupBuses.Count; i++)
                    {
                        ...
                        if (!aBus.isExisting)
                        {
                            ...
                            if (newBypassReverbZones != aBus.bypassReverbZones) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Bypass Reverb Zones");
            ...
            {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Dynamic Playlists");
            ...
            state = _creator.showMusicDucking;
            text = "Dynamic Ducking";
            ...
            if (state != _creator.showMusicDucking) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Dynamic Ducking");
            ...
            state = _creator.showRTPC;
            text = "Dynamic Real Time Parameter Commands";

            DTGUIHelper.ShowCollapsibleSection(ref state, text);

            GUILayout.Space(2f);

            if (state != _creator.showRTPC) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Real Time Parameter Commands");
                _creator.showRTPC = state;
            }

            GUI.color = Color.white;

            DTGUIHelper.AddHelpIconNoStyle("https://www.dtdevtools.com/docs/masteraudio/DynamicSoundGroupCreators.htm#RealTimeParameterCommands");

            EditorGUILayout.EndHorizontal();

            if (_creator.showRTPC) {
                DTGUIHelper.BeginGroupedControls();
                ...
                if (!_creator.showParameterManagement) {
                    labelName += " (" + _creator.realTimeParameters.Count + ")";
                }

                var newShow = DTGUIHelper.Foldout(_creator.showParameterManagement, labelName);
                if (newShow != _creator.showParameterManagement) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Parameter Management");
                    _creator.showParameterManagement = !_creator.showParameterManagement;
                }
                DTGUIHelper.AddHelpIconNoStyle("https://www.dtdevtools.com/docs/masteraudio/DynamicSoundGroupCreators.htm#RealTimeParameterCommands");
                EditorGUILayout.EndHorizontal();
                ...
                if (_creator.showParameterManagement) {
                    ...
                    if (GUILayout.Button(new GUIContent("Add Parameter"), EditorStyles.toolbarButton, GUILayout.Width(100))) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "Add Parameter");
                        var initialName = ("NewParameter " + (_creator.realTimeParameters.Count + 1));
                        _creator.realTimeParameters.Add(new RealTimeParameter {
                            ParameterName = initialName,
                            Value = 0f,
                            IsEditing = false,
                            IsExpanded = true,
                            ProspectiveName = initialName,
                            MinValue = 0f,
                            MaxValue = 1000f
                        });
                    }

                    if (_creator.realTimeParameters.Count > 0) {
                        var hasExpanded = false;
                        foreach (var t in _creator.realTimeParameters) {
                            if (!t.IsExpanded) {
                                continue;
                            }
                        ...
                        if (GUILayout.Button(new GUIContent(theButtonText), EditorStyles.toolbarButton, GUILayout.Width(100))) {
                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "Expand / Collapse All");
                    ...
                    for (var i = 0; i < _creator.realTimeParameters.Count; i++) {
                        var param = _creator.realTimeParameters[i];
                        ...
                        if (!param.IsEditing || Application.isPlaying) {
                            EditorGUI.indentLevel = 1;
                            var newExp = DTGUIHelper.Foldout(param.IsExpanded, param.ParameterName);
                            if (newExp != param.IsExpanded) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle expand Parameter");
                                param.IsExpanded = newExp;
                            }
                        ...
                        if (param.IsExpanded) {
                            ...
                            if (newMin != param.MinValue) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Min Value");
                            ...
                            if (newMax != param.MaxValue) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Max Value");
                    ...
                    if (parameterToRemove.HasValue) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "delete Parameter");
                        _creator.realTimeParameters.RemoveAt(parameterToRemove.Value);
                    }
                    ...
                    if (parameterRenaming != null) {
                        ...
                        if (isValidName) {
                            // ReSharper disable once ForCanBeConvertedToForeach
                            for (var c = 0; c < _creator.realTimeParameters.Count; c++) {
                                var param = _creator.realTimeParameters[c];
                                // ReSharper disable once InvertIf
                ...
                if (!_creator.showParameterCommands) {
                    labelName += " (" + _creator.parameterCommands.Count + ")";
                }
                var newAmb = DTGUIHelper.Foldout(_creator.showParameterCommands, labelName);
                if (newAmb != _creator.showParameterCommands) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Parameter Commands");
                    _creator.showParameterCommands = newAmb;
                }
                DTGUIHelper.AddHelpIconNoStyle("https://www.dtdevtools.com/docs/masteraudio/DynamicSoundGroupCreators.htm#RealTimeParameterCommands");
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.EndVertical();
                ...
                if (_creator.showParameterCommands) {
                    ...
                    if (GUILayout.Button(new GUIContent("Add Parameter Command"), EditorStyles.toolbarButton, GUILayout.Width(160))) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "Add Parameter Command");
                        var initialName = ("NewParameterCommand " + (_creator.parameterCommands.Count + 1));
                        _creator.parameterCommands.Add(new ParameterCommand {
                            CommandName = initialName,
                            IsEditing = false,
                            IsExpanded = true,
                            ProspectiveName = initialName
                        });
                    ...
                    if (_creator.parameterCommands.Count > 0) {
                        var hasExpanded = false;
                        foreach (var t in _creator.parameterCommands) {
                            if (!t.IsExpanded) {
                                continue;
                            }
                    ...
                    for (var i = 0; i < _creator.parameterCommands.Count; i++) {
                        var aParamCmd = _creator.parameterCommands[i];
                        ...
                        if (!aParamCmd.IsEditing || Application.isPlaying) {
                            ...
                            var newExp = DTGUIHelper.Foldout(aParamCmd.IsExpanded, labelName);
                            if (newExp != aParamCmd.IsExpanded) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle expand Parameter Command");
                                aParamCmd.IsExpanded = newExp;
                            }
                        ...
                        if (aParamCmd.IsExpanded) {
                            ...
                            if (newIndex >= 0) {
                                if (index != newIndex) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Parameter");
                                }
                            ...
                            if (!isParamValid) {
                                ...
                                if (!MasterAudio.ParameterCommandHardCodedNames.Contains(aParamCmd.ParameterName)) {
                                    selectedParam = GetRealTimeParameterFromCommand(aParamCmd);
                                }
                                ...
                                if (selectedParam == null) {
                                    ...
                                    if (newMinParamVal != aParamCmd.MinDisplayCurveX) {
                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Curve X Min");
                                    ...
                                    if (newMaxParamVal != aParamCmd.MaxDisplayCurveX) {
                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Curve X Max");
                                ...
                                if (GUILayout.Button(new GUIContent("Add Element"), EditorStyles.toolbarButton, GUILayout.Width(110))) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "Add Element");
                                    aParamCmd.Elements.Add(new ParameterCommandElement {
                                        ElementName = "YourElement" + (aParamCmd.Elements.Count + 1),
                                        VariationName = "YourVariation",
                                        IsExpanded = true,
                                        IsEditing = false,
                                        ShowAdvanced = false,
                                        VariationMode = EventSounds.VariationType.PlayRandom,
                                        ...
                                        FadeInRangeStart = 0f,
                                        FadeInRangeEnd = 1f,
                                        FadeOutRangeStart = 9f,
                                        FadeOutRangeEnd = 10f,
                                        SoundSpawnLocationMode = MasterAudio.SoundSpawnLocationMode.AttachToCaller,
                                        ShowCurves = true,
                                        AffectVolume = false,
                                        VolumeAnimationCurve = new AnimationCurve(),
                                        AffectPitch = false,
                                        PitchAnimationCurve = new AnimationCurve(),
                                        ShowFadeInOutFields = false,
                                        MaxDisplayVolumeCurveY = 1f,
                                        AffectHighPassFilter = false,
                                        LowPassAnimationCurve = new AnimationCurve(),
                                        AffectLowPassFilter = false,
                                        HighPassAnimationCurve = new AnimationCurve(),
                                        ShowSphereGizmo = false
                                    });
                                }
                                ...
                                for (var d = 0; d < aParamCmd.Elements.Count; d++) {
                                    ...
                                    if (!element.IsEditing || Application.isPlaying) {
                                        ...
                                        var newExp = DTGUIHelper.Foldout(element.IsExpanded, elementLabel);
                                        if (newExp != element.IsExpanded) {
                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle expand Element");
                                            element.IsExpanded = newExp;
                                        }
                                    ...
                                    if (element.IsExpanded) {
                                        ...
                                        if (isValidElementSound) {
                                            ...
                                            if (newShowCurves != element.ShowCurves) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Show Animation Curves");
                                            ...
                                            if (element.ShowCurves) {
                                                ...
                                                if (newAffectVolume != element.AffectVolume) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Affect Volume");
                                                ...
                                                if (element.AffectVolume) {
                                                    ...
                                                    if (newVolCurve != element.VolumeAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Volume Curve");
                                                    ...
                                                    if (newShowRange != element.ShowFadeInOutFields) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Fade In/Out Fields");
                                                    ...
                                                    if (element.ShowFadeInOutFields) {
                                                        ...
                                                        if (newFadeInStart != element.FadeInRangeStart) {
                                                            AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change " + label);
                                                        ...
                                                        label = "Max Volume";
                                                        var newMaxVol = EditorGUILayout.FloatField(label, element.MaxDisplayVolumeCurveY);
                                                ...
                                                if (newAffectPitch != element.AffectPitch) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Affect Pitch");
                                                ...
                                                if (element.AffectPitch) {
                                                    ...
                                                    if (newPitchCurve != element.PitchAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Pitch Curve");
                                                ...
                                                if (newAffectLowPass != element.AffectLowPassFilter) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Affect Low Pass Filter");
                                                ...
                                                if (element.AffectLowPassFilter) {
                                                    ...
                                                    if (newLowPassCurve != element.LowPassAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Low Pass Filter Curve");
                                                ...
                                                if (newAffectHighPass != element.AffectHighPassFilter) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Affect High Pass Filter");
                                                ...
                                                if (element.AffectHighPassFilter) {
                                                    ...
                                                    if (newHighPassCurve != element.HighPassAnimationCurve) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change High Pass Filter Curve");
                                        ...
                                        if (elementNewSoundGroupIndex >= 0) {
                                            if (elementSoundGroupIndex != elementNewSoundGroupIndex) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Element Sound Group");
                                        ...
                                        if (!isValidElementSound) {
                                            ...
                                            if (newShowAdv != element.ShowAdvanced) {
                                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Advanced Controls");
                                            ...
                                            if (element.ShowAdvanced) {
                                                ...
                                                if (element.VariationMode == EventSounds.VariationType.PlaySpecific) {
                                                    ...
                                                    if (newVariation != element.VariationName) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Variation Name");
                                                ...
                                                if (newSpawnMode != element.SoundSpawnLocationMode) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Sound Spawn Mode");
                                                ...
                                                if (!element.AffectPitch) {
                                                    ...
                                                    if (newUseSpecificPitch != element.OverridePitch) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Override Pitch");
                                                ...
                                                if (aud != null) {
                                                    ...
                                                    if (newShowgiz != element.ShowSphereGizmo) {
                                                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "toggle Adjust Audio Range");
                                                        if (newShowgiz == true) {
                                                            // turn rest off
                                                            TurnOffAllOtherShowRangeGizmo();

                                                            _creator.showParameterCommandGizmo = true;
                                                            _creator.parameterCommandToGizmo = element;
                                                        }

                                                        if ((_creator.parameterCommandToGizmo == element) && !newShowgiz) {
                                                            _creator.showParameterCommandGizmo = false;
                                                            _creator.parameterCommandToGizmo = null;
                                                        }
                                                        ...
                                                        if (newMin != aud.minDistance) {
                                                            ...
                                                            switch (element.VariationMode) {
                                                                case EventSounds.VariationType.PlayRandom:
                                                                    var sources = _creator.GetAllVariationAudioSources(element);
                                                                    if (sources != null) {
                                                ...
                                                if (newDelay != element.DelaySound) {
                                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "change Delay Sound");
                                ...
                                if (elementToRemove.HasValue) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "delete Element");
                    ...
                    if (paramCmdToRemove.HasValue) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _creator, "delete Parameter Command");
                        _creator.parameterCommands.RemoveAt(paramCmdToRemove.Value);
                    }

                    if (parameterCommandEditing != null) {
                        // ReSharper disable once ForCanBeConvertedToForeach
                        for (var c = 0; c < _creator.parameterCommands.Count; c++) {
                            var anEvent = _creator.parameterCommands[c];
                            // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
        ...
        private void CalculateRadiusIfSelected(ParameterCommandElement element) {
            if (element.ShowSphereGizmo) {
                _creator.CalculateRadius(element);
            }
        }
        ...
        private void TurnOffAllOtherShowRangeGizmo() {
            foreach (var paramCmd in _creator.parameterCommands) {
        ...
        private List<string> ParameterNameList {
            get {
                ...
                foreach (var p in _creator.realTimeParameters) {
        ...
        private RealTimeParameter GetRealTimeParameterFromCommand(ParameterCommand cmd) {
            return GetRealTimeParameterByName(cmd.ParameterName);
        }
        
        public RealTimeParameter GetRealTimeParameterByName(string paramName) {
            RealTimeParameter selectedParam = null;
            for (var i = 0; i < _creator.realTimeParameters.Count; i++) {
                var aParam = _creator.realTimeParameters[i];
                if (aParam.ParameterName == paramName) {
                    selectedParam = aParam;
                    break;
                }
            }
