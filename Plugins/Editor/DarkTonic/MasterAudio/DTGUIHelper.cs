...
namespace DarkTonic.MasterAudio.EditorScripts
{
    ...
    public static void PreviewAddressable(AssetReference assetRef, AudioSource previewer, float volume) {
        ...
        public static JukeboxButtons AddJukeboxIcons(PlaylistController playlistController)
        {
            ...
            switch (playlistController.PlaylistState) {
                case PlaylistController.PlaylistStates.Paused:
                    var pausedIcon = MasterAudioInspectorResources.PauseOnTexture;
                    var pausedContent = pausedIcon == null ? new GUIContent("Pause", "Pause Playlist") : new GUIContent(pausedIcon, "Pause Playlist");
                    buttonWidth = pausedIcon == null ? 50 : 30;
                    if (GUILayout.Button(pausedContent, EditorStyles.toolbarButton, GUILayout.MaxWidth(buttonWidth))) {
        ...
        public static DTFunctionButtons AddMixerBusButtons(GroupBus gb)
        {
            ...
            var stopIcon = MasterAudioInspectorResources.StopTexture;
            var pauseIcon = MasterAudioInspectorResources.PauseTexture;
            var unpauseIcon = MasterAudioInspectorResources.UnpauseTexture;
            ...
            var unpausePressed = false;

            if (Application.isPlaying)
            {
                if (gb.IsPaused) {
                    unpausePressed = GUILayout.Button(new GUIContent(unpauseIcon, "Click to unpause Bus"), EditorStyles.toolbarButton);
                } else {
                    pausePressed = GUILayout.Button(new GUIContent(pauseIcon, "Click to pause Bus"), EditorStyles.toolbarButton);
                }
            ...
            if (pausePressed) {
                return DTFunctionButtons.Pause;
            } else if (unpausePressed) {
                return DTFunctionButtons.Unpause;
            }
        ...
        public static DTFunctionButtons AddMasterPlaylistButtons(string itemName, MasterAudio sounds)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                if (allPaused) {
                    var unpauseContent = new GUIContent(MasterAudioInspectorResources.PauseOnTexture, "Click to unpause " + itemName);
                    if (GUILayout.Button(unpauseContent, EditorStyles.toolbarButton)) {
                        return DTFunctionButtons.Play;
                    }
                } else {
                    var pauseContent = new GUIContent(MasterAudioInspectorResources.PauseTexture, "Click to pause " + itemName);
        ...
        public static DTFunctionButtons AddMixerButtons(MasterAudioGroup aGroup, string itemName, bool showSettingsIcon = true)
        {
            ...
            if (Application.isPlaying) {
                if (aGroup.IsPaused) {
                    if (GUILayout.Button(new GUIContent(unpauseIcon, "Click to unpause Bus"), EditorStyles.toolbarButton)) {
                        return DTFunctionButtons.Unpause;
                    }
                } else {
                    if (GUILayout.Button(new GUIContent(pauseIcon, "Click to pause Bus"), EditorStyles.toolbarButton)) {
                        return DTFunctionButtons.Pause;
                    }
                }
            }
