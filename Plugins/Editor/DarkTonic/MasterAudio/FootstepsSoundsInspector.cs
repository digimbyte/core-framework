...
namespace DarkTonic.MasterAudio.EditorScripts
{
    ...
    public class FootstepsSoundsInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            List<string> _parameterCmdNames;

            if (maInScene)
            {
                DTGUIHelper.ShowHeaderTexture(MasterAudioInspectorResources.LogoTexture);
                _groupNames = ma.GroupNames;
                _parameterCmdNames = ma.ParameterCommandNames;
            } else
            {
                _groupNames = new List<string>();
                _parameterCmdNames = new List<string>();
            }
            ...
            for (var f = 0; f < _sounds.footstepGroups.Count; f++)
            {
                ...
                var newPlaySoundType = (EventSounds.PlaySoundType)EditorGUILayout.EnumPopup("Action Type", step.playSoundType);
                if (newPlaySoundType != step.playSoundType) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Action Type");
                    step.playSoundType = newPlaySoundType;
                }

                switch (step.playSoundType) {
                    case EventSounds.PlaySoundType.PlaySoundGroup:
                        if (maInScene) {
                        ...
                        break;
                    case EventSounds.PlaySoundType.InvokeParameterCommand:
                        if (maInScene) {
                            var existingIndex = _parameterCmdNames.IndexOf(step.parameterCommandName);
                            ...
                            if (existingIndex >= 1) {
                                ...
                                EditorGUILayout.EndHorizontal();
                            } else if (existingIndex == -1 && step.parameterCommandName == MasterAudio.NoGroupName) {
                                paramCmdIndex = EditorGUILayout.Popup("Parameter Command", existingIndex, _parameterCmdNames.ToArray());
                            } else { // non-match
                                noMatch = true;
                                var newCommand = EditorGUILayout.TextField("Parameter Command", step.parameterCommandName);
                                if (newCommand != step.parameterCommandName) {
                                    AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Command");
                                    step.parameterCommandName = newCommand;
                                }
                            ...
                            if (paramCmdIndex.HasValue) {
                                ...
                                switch (paramCmdIndex.Value) {
                                    case -1:
                                        step.parameterCommandName = MasterAudio.NoGroupName;
                                        break;
                                    default:
                                        step.parameterCommandName = _parameterCmdNames[paramCmdIndex.Value];
                                        break;
                                }
                            }
                        } else {
                            var newParamCmd = EditorGUILayout.TextField("Parameter Command", step.parameterCommandName);
                            if (newParamCmd != step.parameterCommandName) {
                                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Parameter Command");
                                step.parameterCommandName = newParamCmd;
                            }
                        }
